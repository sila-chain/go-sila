...
import (
	...
	"github.com/sila-chain/go-sila/crypto/signify"
	"github.com/sila-chain/go-sila/internal/build"
	"github.com/sila-chain/go-sila/internal/download"
...
var (
	...
	// Files that end up in the gsil-alltools*.zip archive (and the NSIS installer
...
// gsil-alltools-{platform}-{ver}.zip archive, with binary paths resolved for
// the target OS.
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string, targetOS string) (flags []string) {
	...
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/sila-chain/go-sila/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/sila-chain/go-sila/internal/version.gitDate="+env.Date)
	}
...
func doCheckBadDeps() {
	baddeps := [][2]string{
		...
		{"github.com/sila-chain/go-sila/core/rawdb", "github.com/sila-chain/go-sila/sildb/leveldb"},
		{"github.com/sila-chain/go-sila/core/rawdb", "github.com/sila-chain/go-sila/sildb/pebbledb"},
	}
	tc := new(build.GoToolchain)
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gsil-release.pub"
...
func doDockerBuildx(cmdline []string) {
	var (
		...
		hubImage = flag.String("hub", "sila/client-go", `Where to upload the docker image`)
	...
	//  - sila/client-go:latest                            - Pushes to the master branch, Sila only
	//  - sila/client-go:stable                            - Version tag publish on GitHub, Sila only
	//  - sila/client-go:alltools-latest                   - Pushes to the master branch, Sila & tools
	//  - sila/client-go:alltools-stable                   - Version tag publish on GitHub, Sila & tools
	//  - sila/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Sila only
	//  - sila/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Sila & tools
	//  - sila/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Sila only
	//  - sila/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Sila & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "sila/sila")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gsil-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for _, distro := range debDistros {
			// Prepare the debian package with the go-sila sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gsil-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debMetadata struct {
	...
	// go-sila version being built. Note that this
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Sila Builds <fjl@sila.org>"
	}
	return debMetadata{
		PackageName: name,
		Env:         env,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	installer, err := filepath.Abs("gsil-" + archiveBasename("windows", *arch, version.Archive(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand(makensisCmd,
		...
		filepath.Join(*workdir, "gsil.nsi"),
	)
