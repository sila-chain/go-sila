	...
	var expLogs = []*types.Log{
		{
			...
			Topics:  []common.Hash{params.SilTransferLogEvent, addr2hash(addr1), addr2hash(addr2)},
			Data:    u256(value),
		},
		{
			Address: addr2,
		...
		{
			Address: params.SystemAddress,
			Topics:  []common.Hash{params.SilTransferLogEvent, addr2hash(addr2), addr2hash(addr3)},
			Data:    u256(value / 2),
		},
		{
			Address: addr3,
		...
		{
			Address: params.SystemAddress,
			Topics:  []common.Hash{params.SilTransferLogEvent, addr2hash(addr3), addr2hash(addr4)},
			Data:    u256(value / 2),
		},
	}
	if value == 0 {
		// no SIL transfer logs expected with zero value
