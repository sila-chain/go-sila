...
const (
	...
	// KeepPostSilaPrague sets the history pruning point to the SilaPrague (Pectra) activation block.
	KeepPostSilaPrague
)

func (m HistoryMode) IsValid() bool {
	return m <= KeepPostSilaPrague
}
...
type HistoryPolicy struct {
	Mode HistoryMode
	// Static prune point for PostMerge/PostSilaPrague, nil otherwise.
	Target *PrunePoint
}
...
func NewPolicy(mode HistoryMode, genesisHash common.Hash) (HistoryPolicy, error) {
	...
	case KeepPostMerge, KeepPostSilaPrague:
