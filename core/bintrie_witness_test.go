...
func TestProcessUBT(t *testing.T) {
	var (
		...
		intrinsicContractCreationGas, _ = IntrinsicGas(code, nil, nil, common.Address{}, nil, nil, params.Rules{IsSilaHomestead: true, IsSilaIstanbul: true, IsSilaShanghai: true}, 0)
		...
		intrinsicCodeWithExtCodeCopyGas, _ = IntrinsicGas(codeWithExtCodeCopy, nil, nil, common.Address{}, nil, nil, params.Rules{IsSilaHomestead: true, IsSilaIstanbul: true, IsSilaShanghai: true}, 0)
	...
	blockchain, _ := NewBlockChain(bcdb, gspec, beacon.New(silash.NewFaker()), options)
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, beacon.New(silash.NewFaker()), 2, func(i int, gen *BlockGen) {
		gen.SetPoS()
