...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, silash.NewFaker(), b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, gspec, silash.NewFaker(), nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	genesis := &Genesis{Config: params.AllSilashProtocolChanges}
	for b.Loop() {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for b.Loop() {
		...
		chain, err := NewBlockChain(db, genesis, silash.NewFaker(), options)
		if err != nil {
