...
func NewHeaderChain(chainDb sildb.Database, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	hc := &HeaderChain{
		config:        config,
		chainDb:       chainDb,
...
type (
	...
	UpdateHeadBlocksCallback func(sildb.KeyValueWriter, *types.Header) (*types.Header, bool)

	// DeleteBlockContentCallback is a callback function that is called by SetHead
	// before each header is deleted.
	DeleteBlockContentCallback func(sildb.KeyValueWriter, common.Hash, uint64)
)
