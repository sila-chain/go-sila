...
// silash tests, please use OffsetTime, which implicitly recalculates the diff.
...
func (b *BlockGen) AddUncle(h *types.Header) {
	...
	if b.cm.config.IsSilaLondon(h.Number) {
		h.BaseFee = sip1559.CalcBaseFee(b.cm.config, parent)
		if !b.cm.config.IsSilaLondon(parent.Number) {
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db sildb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
...
func GenerateChainWithGenesis(genesis *Genesis, engine consensus.Engine, n int, gen func(int, *BlockGen)) (sildb.Database, []*types.Block, []types.Receipts) {
...
func (cm *chainMaker) makeHeader(parent *types.Block, engine consensus.Engine) *types.Header {
	...
	if cm.config.IsSilaCancun(header.Number, header.Time) {
...
func makeHeaderChain(chainConfig *params.ChainConfig, parent *types.Header, n int, engine consensus.Engine, db sildb.Database, seed int) []*types.Header {
...
func makeHeaderChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine, seed int) (sildb.Database, []*types.Header) {
...
func makeBlockChain(chainConfig *params.ChainConfig, parent *types.Block, n int, engine consensus.Engine, db sildb.Database, seed int) []*types.Block {
...
func makeBlockChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine, seed int) (sildb.Database, []*types.Block) {
