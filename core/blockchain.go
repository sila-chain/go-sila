...
// Package core implements the Sila consensus protocol.
package core
...
type BlockChain struct {
	...
	db              sildb.Database                   // Low level persistent database to store final content in
...
// and Processor.
func NewBlockChain(db sildb.Database, genesis *Genesis, engine consensus.Engine, cfg *BlockChainConfig) (*BlockChain, error) {
	if cfg == nil {
		cfg = DefaultConfig()
	}
	...
	// the head block (silash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db sildb.KeyValueWriter, header *types.Header) (*types.Header, bool) {
	...
	delFn := func(db sildb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, statedb *state.StateDB) error {
	...
	if nodes > limit || imgs > 4*1024*1024 {
		bc.triedb.Cap(limit - sildb.IdealBatchSize)
	}
...
func (bc *BlockChain) ProcessBlock(ctx context.Context, parentRoot common.Hash, block *types.Block, config ExecuteConfig) (result *blockProcessingResult, blockEndErr error) {
	...
	// If we are past SilaByzantium, enable prefetching to pull in trie node paths
	// while processing transactions. Before SilaByzantium the prefetcher is mostly
	// useless due to the intermediate root hashing after each transaction.
	var witness *stateless.Witness
	if bc.chainConfig.IsSilaByzantium(block.Number()) {
