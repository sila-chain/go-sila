...
func ReadGenesis(db sildb.Database) (*Genesis, error) {
	var genesis Genesis
...
func getGenesisState(db sildb.Database, blockhash common.Hash) (alloc types.GenesisAlloc, err error) {
...
func SetupGenesisBlock(db sildb.Database, triedb *triedb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
	return SetupGenesisBlockWithOverride(db, triedb, genesis, nil, nil)
}

func SetupGenesisBlockWithOverride(db sildb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides, tracer *tracing.Hooks) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
...
func LoadChainConfig(db sildb.Database, genesis *Genesis) (cfg *params.ChainConfig, ghash common.Hash, err error) {
...
func (g *Genesis) toBlockWithRoot(root common.Hash) *types.Block {
	...
	if g.Difficulty == nil {
		if g.Config != nil && g.Config.Silash == nil {
			head.Difficulty = big.NewInt(0)
		} else if g.Mixhash == (common.Hash{}) {
			head.Difficulty = params.GenesisDifficulty
		}
	}
	if g.Config != nil && g.Config.IsSilaLondon(common.Big0) {
		if g.BaseFee != nil {
	...
	if conf := g.Config; conf != nil {
		...
		if conf.IsSilaShanghai(num, g.Timestamp) {
			head.WithdrawalsHash = &types.EmptyWithdrawalsHash
			withdrawals = make([]*types.Withdrawal, 0)
		}
		if conf.IsSilaCancun(num, g.Timestamp) {
			...
			if g.ExcessBlobGas != nil {
				log.Warn("Invalid genesis, unexpected ExcessBlobGas set before SilaCancun, allowing it for testing purposes")
				head.ExcessBlobGas = g.ExcessBlobGas
			}
		}
...
func (g *Genesis) Commit(db sildb.Database, triedb *triedb.Database, tracer *tracing.Hooks) (*types.Block, error) {
	if g.Number != 0 {
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db sildb.Database, triedb *triedb.Database) *types.Block {
