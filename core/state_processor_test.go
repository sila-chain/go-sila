...
func TestStateProcessorErrors(t *testing.T) {
	...
	{ // Tests against a 'recent' chain definition
		...
		for i, tt := range []struct {
			...
			{ // ErrEmptyAuthList
				...
				want: "could not apply tx 0 [0xc18d10f4c809dbdfa1a074c3300de9bc4b7f16a20f0ec667f6f67312b71b956a]: SIP-7702 transaction with empty auth list (sender 0x71562b71999873DB5b286dF957af199Ec94617F7)",
			},
			...
			block := GenerateBadBlock(gspec.ToBlock(), beacon.New(silash.NewFaker()), tt.txs, gspec.Config, false)
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		var (
			...
			blockchain, _ = NewBlockChain(db, gspec, silash.NewFaker(), nil)
		)
		defer blockchain.Stop()
		for i, tt := range []struct {
			txs  []*types.Transaction
			...
			block := GenerateBadBlock(gspec.ToBlock(), silash.NewFaker(), tt.txs, gspec.Config, true)
			_, err := blockchain.InsertChain(types.Blocks{block})
			if err == nil {
				t.Fatal("block imported without errors")
			}
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions, config *params.ChainConfig, isPOW bool) *types.Block {
	...
	if config.IsSilaLondon(header.Number) {
		header.BaseFee = sip1559.CalcBaseFee(config, parent.Header())
	}
	if config.IsSilaShanghai(header.Number, header.Time) {
	...
	if config.IsSilaCancun(header.Number, header.Time) {
