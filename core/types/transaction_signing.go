...
func MakeSigner(config *params.ChainConfig, blockNumber *big.Int, blockTime uint64) Signer {
	var signer Signer
	switch {
	case config.IsSilaPrague(blockNumber, blockTime):
		signer = NewSilaPragueSigner(config.ChainID)
	case config.IsSilaCancun(blockNumber, blockTime):
		signer = NewSilaCancunSigner(config.ChainID)
...
func LatestSigner(config *params.ChainConfig) Signer {
	...
	if config.ChainID != nil {
		switch {
		case config.SilaPragueTime != nil:
			signer = NewSilaPragueSigner(config.ChainID)
		case config.SilaCancunTime != nil:
			signer = NewSilaCancunSigner(config.ChainID)
...
func LatestSignerForChainID(chainID *big.Int) Signer {
	var signer Signer
	if chainID != nil {
		signer = NewSilaPragueSigner(chainID)
	} else {
		signer = SilaHomesteadSigner{}
	}
	return signer
...
func newModernSigner(chainID *big.Int, fork forks.Fork) Signer {
	...
	// configure tx types
	if fork >= forks.SilaBerlin {
		s.txtypes.set(AccessListTxType)
	}
	if fork >= forks.SilaLondon {
...
func (s *modernSigner) Sender(tx *Transaction) (common.Address, error) {
	...
	// id, add 27 to become equivalent to unprotected SilaHomestead signatures.
...
// NewSilaPragueSigner returns a signer that accepts
...
// - legacy SilaHomestead transactions.
func NewSilaPragueSigner(chainId *big.Int) Signer {
	return newModernSigner(chainId, forks.SilaPrague)
}

// NewSilaCancunSigner returns a signer that accepts
// - SIP-4844 blob transactions
// - SIP-1559 dynamic fee transactions
// - SIP-2930 access list transactions,
// - SIP-155 replay protected transactions, and
// - legacy SilaHomestead transactions.
func NewSilaCancunSigner(chainId *big.Int) Signer {
	return newModernSigner(chainId, forks.SilaCancun)
}

// NewSilaLondonSigner returns a signer that accepts
// - SIP-1559 dynamic fee transactions
// - SIP-2930 access list transactions,
// - SIP-155 replay protected transactions, and
// - legacy SilaHomestead transactions.
func NewSilaLondonSigner(chainId *big.Int) Signer {
...
func (s SIP155Signer) Sender(tx *Transaction) (common.Address, error) {
	...
	if !tx.Protected() {
		return SilaHomesteadSigner{}.Sender(tx)
	}
	if tx.ChainId().Cmp(s.chainId) != 0 {
		return common.Address{}, fmt.Errorf("%w: have %d want %d", ErrInvalidChainId, tx.ChainId(), s.chainId)
	}
...
func (hs SilaHomesteadSigner) ChainID() *big.Int {
	return nil
}

func (hs SilaHomesteadSigner) Equal(s2 Signer) bool {
	_, ok := s2.(SilaHomesteadSigner)
	return ok
}

// SignatureValues returns signature values. This signature
// needs to be in the [R || S || V] format where V is 0 or 1.
func (hs SilaHomesteadSigner) SignatureValues(tx *Transaction, sig []byte) (r, s, v *big.Int, err error) {
	return hs.FrontierSigner.SignatureValues(tx, sig)
}

func (hs SilaHomesteadSigner) Sender(tx *Transaction) (common.Address, error) {
	if tx.Type() != LegacyTxType {
		return common.Address{}, ErrTxTypeNotSupported
	}
	v, r, s := tx.RawSignatureValues()
