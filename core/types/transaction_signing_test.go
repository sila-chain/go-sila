...
func TestSIP155Signing(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)

	signer := NewSIP155Signer(big.NewInt(18))
...
func TestSIP155ChainId(t *testing.T) {
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)
	...
	tx, err = SignTx(tx, SilaHomesteadSigner{}, key)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSIP155SigningVitalik(t *testing.T) {
	// Test vectors come from http://vitalik.ca/files/sip155_testvec.txt
	for i, test := range []struct {
		...
		signer := NewSIP155Signer(big.NewInt(1))
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTx(tx, NewSIP155Signer(big.NewInt(1)), key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(NewSIP155Signer(big.NewInt(2)), tx)
	if !errors.Is(err, ErrInvalidChainId) {
		t.Error("expected error:", ErrInvalidChainId, err)
	}

	_, err = Sender(NewSIP155Signer(big.NewInt(1)), tx)
	if err != nil {
