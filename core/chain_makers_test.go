...
func TestGeneratePOSChain(t *testing.T) {
	var (
		...
		funds   = big.NewInt(0).Mul(big.NewInt(1337), big.NewInt(params.Sila))
...
func ExampleGenerateChain() {
	...
	gspec := &Genesis{
		Config: &params.ChainConfig{SilaHomesteadBlock: new(big.Int)},
	...
	chain, _ := GenerateChain(gspec.Config, genesis, silash.NewFaker(), genDb, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
			// In block 1, addr1 sends addr2 some sila.
			tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1), addr2, big.NewInt(10000), params.TxGas, nil, nil), signer, key1)
			gen.AddTx(tx)
		case 1:
			// In block 2, addr1 sends some more sila to addr2.
	...
	// Import the chain. This runs all block validation rules.
	blockchain, _ := NewBlockChain(db, gspec, silash.NewFaker(), DefaultConfig().WithStateScheme(rawdb.HashScheme))
	defer blockchain.Stop()
