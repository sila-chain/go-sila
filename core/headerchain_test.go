...
func TestHeaderInsertion(t *testing.T) {
	var (
		...
		gspec = &Genesis{BaseFee: big.NewInt(params.InitialBaseFee), Config: params.AllSilashProtocolChanges}
	)
	gspec.Commit(db, triedb.NewDatabase(db, nil), nil)
	hc, err := NewHeaderChain(db, gspec.Config, silash.NewFaker(), func() bool { return false })
	if err != nil {
		t.Fatal(err)
	}
	// chain A: G->A1->A2...A128
	genDb, chainA := makeHeaderChainWithGenesis(gspec, 128, silash.NewFaker(), 10)
	// chain B: G->A1->B1...B128
	chainB := makeHeaderChain(gspec.Config, chainA[0], 128, silash.NewFaker(), genDb, 10)
