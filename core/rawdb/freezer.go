...
func (f *Freezer) ReadAncients(fn func(sildb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(sildb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
