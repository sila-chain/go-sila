...
func (f *resettableFreezer) ReadAncients(fn func(sildb.AncientReaderOp) error) (err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *resettableFreezer) ModifyAncients(fn func(sildb.AncientWriteOp) error) (writeSize int64, err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
