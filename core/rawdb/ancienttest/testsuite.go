...
// implementation.
func TestAncientSuite(t *testing.T, newFn func(kinds []string) sildb.AncientStore) {
...
func basicRead(t *testing.T, newFn func(kinds []string) sildb.AncientStore) {
	var (
		db   = newFn([]string{"a"})
	...
	if _, err := db.ModifyAncients(func(op sildb.AncientWriteOp) error {
...
func batchRead(t *testing.T, newFn func(kinds []string) sildb.AncientStore) {
	var (
		db   = newFn([]string{"a"})
		data = makeDataset(100, 32)
	)
...
func basicWrite(t *testing.T, newFn func(kinds []string) sildb.AncientStore) {
	var (
		db    = newFn([]string{"a", "b"})
	...
	size, err := db.ModifyAncients(func(op sildb.AncientWriteOp) error {
		for i := 0; i < 100; i++ {
			if err := op.AppendRaw("a", uint64(i), dataA[i]); err != nil {
				return err
			}
...
func nonMutable(t *testing.T, newFn func(kinds []string) sildb.AncientStore) {
	db := newFn([]string{"a"})
	defer db.Close()
...
func TestResettableAncientSuite(t *testing.T, newFn func(kinds []string) sildb.ResettableAncientStore) {
