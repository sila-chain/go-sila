...
func ReadPreimage(db sildb.KeyValueReader, hash common.Hash) []byte {
...
func WritePreimages(db sildb.KeyValueWriter, preimages map[common.Hash][]byte) {
...
func ReadCode(db sildb.KeyValueReader, hash common.Hash) []byte {
...
func ReadCodeWithPrefix(db sildb.KeyValueReader, hash common.Hash) []byte {
...
func HasCode(db sildb.KeyValueReader, hash common.Hash) bool {
	// Try with the prefixed code scheme first, if not then try with legacy
	// scheme.
...
func HasCodeWithPrefix(db sildb.KeyValueReader, hash common.Hash) bool {
	ok, _ := db.Has(codeKey(hash))
	return ok
}

// WriteCode writes the provided contract code database.
func WriteCode(db sildb.KeyValueWriter, hash common.Hash, code []byte) {
...
func DeleteCode(db sildb.KeyValueWriter, hash common.Hash) {
...
func ReadStateID(db sildb.KeyValueReader, root common.Hash) *uint64 {
...
func WriteStateID(db sildb.KeyValueWriter, root common.Hash, id uint64) {
	var buff [8]byte
...
func ReadPersistentStateID(db sildb.KeyValueReader) uint64 {
...
func WritePersistentStateID(db sildb.KeyValueWriter, number uint64) {
...
// the last shutdown.
func ReadTrieJournal(db sildb.KeyValueReader) []byte {
...
// shutdown.
func WriteTrieJournal(db sildb.KeyValueWriter, journal []byte) {
...
// state).
func ReadStateHistoryMeta(db sildb.AncientReaderOp, id uint64) []byte {
...
// state).
func ReadStateHistoryMetaList(db sildb.AncientReaderOp, start uint64, count uint64) ([][]byte, error) {
...
func ReadStateAccountIndex(db sildb.AncientReaderOp, id uint64) []byte {
...
// history in freezer by minus one since the id of first state history starts
// from one (zero for initial state).
func ReadStateStorageIndex(db sildb.AncientReaderOp, id uint64, offset, length int) ([]byte, error) {
...
func ReadStateAccountHistory(db sildb.AncientReaderOp, id uint64, offset, length int) ([]byte, error) {
...
// one since the id of first state history starts from one (zero for initial
// state).
func ReadStateStorageHistory(db sildb.AncientReaderOp, id uint64, offset, length int) ([]byte, error) {
...
func ReadStateHistory(db sildb.AncientReaderOp, id uint64) ([]byte, []byte, []byte, []byte, []byte, error) {
...
func ReadStateHistoryList(db sildb.AncientReaderOp, start uint64, count uint64) ([][]byte, [][]byte, [][]byte, [][]byte, [][]byte, error) {
...
func WriteStateHistory(db sildb.AncientWriter, id uint64, meta []byte, accountIndex []byte, storageIndex []byte, accounts []byte, storages []byte) error {
	_, err := db.ModifyAncients(func(op sildb.AncientWriteOp) error {
...
func ReadTrienodeHistory(db sildb.AncientReaderOp, id uint64) ([]byte, []byte, []byte, error) {
...
func ReadTrienodeHistoryHeader(db sildb.AncientReaderOp, id uint64) ([]byte, error) {
...
func ReadTrienodeHistoryKeySection(db sildb.AncientReaderOp, id uint64, offset uint64, length uint64) ([]byte, error) {
...
func ReadTrienodeHistoryValueSection(db sildb.AncientReaderOp, id uint64, offset uint64, length uint64) ([]byte, error) {
...
func ReadTrienodeHistoryList(db sildb.AncientReaderOp, start uint64, count uint64) ([][]byte, [][]byte, [][]byte, error) {
...
// the id of first state history starts from one(zero for initial state).
func WriteTrienodeHistory(db sildb.AncientWriter, id uint64, header []byte, keySection []byte, valueSection []byte) error {
	_, err := db.ModifyAncients(func(op sildb.AncientWriteOp) error {
