...
// MemoryFreezer is an ephemeral ancient store. It implements the sildb.AncientStore
...
func (f *MemoryFreezer) ReadAncients(fn func(sildb.AncientReaderOp) error) (err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *MemoryFreezer) ModifyAncients(fn func(sildb.AncientWriteOp) error) (writeSize int64, err error) {
	f.lock.Lock()
	defer f.lock.Unlock()
