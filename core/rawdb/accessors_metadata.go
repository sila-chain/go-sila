...
func ReadDatabaseVersion(db sildb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db sildb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db sildb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db sildb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadGenesisStateSpec(db sildb.KeyValueReader, blockhash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(blockhash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db sildb.KeyValueWriter, blockhash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db sildb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db sildb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
...
func UpdateUncleanShutdownMarker(db sildb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
