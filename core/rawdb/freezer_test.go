...
func TestFreezerConcurrentModifyTruncate(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		wg.Add(3)
		go func() {
			_, modifyErr = f.ModifyAncients(func(op sildb.AncientWriteOp) error {
...
func TestChainFreezerBALAlignment(t *testing.T) {
	...
	if _, err := f.ModifyAncients(func(op sildb.AncientWriteOp) error {
		for i := uint64(0); i < items; i++ {
...
func TestFreezerSuite(t *testing.T) {
	ancienttest.TestAncientSuite(t, func(kinds []string) sildb.AncientStore {
	...
	ancienttest.TestResettableAncientSuite(t, func(kinds []string) sildb.ResettableAncientStore {
		tables := make(map[string]freezerTableConfig)
		for _, kind := range kinds {
			tables[kind] = freezerTableConfig{
				noSnappy:  true,
