...
func ReadSkeletonSyncStatus(db sildb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db sildb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db sildb.KeyValueWriter) {
...
func ReadSkeletonHeader(db sildb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db sildb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db sildb.KeyValueWriter, number uint64) {
...
func ReadSnapSyncStatusFlag(db sildb.KeyValueReader) uint8 {
...
func WriteSnapSyncStatusFlag(db sildb.KeyValueWriter, flag uint8) {
