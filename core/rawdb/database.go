...
func (db *nofreezedb) ModifyAncients(func(sildb.AncientWriteOp) error) (int64, error) {
	return 0, errNotSupported
}
...
func (db *nofreezedb) ReadAncients(fn func(reader sildb.AncientReaderOp) error) (err error) {
...
func NewDatabase(db sildb.KeyValueStore) sildb.Database {
...
//
// Deprecated: use Open.
func NewDatabaseWithFreezer(db sildb.KeyValueStore, ancient string, namespace string, readonly bool) (sildb.Database, error) {
...
func Open(db sildb.KeyValueStore, opts OpenOptions) (sildb.Database, error) {
...
func NewMemoryDatabase() sildb.Database {
...
func InspectDatabase(db sildb.Database, keyPrefix, keyStart []byte) error {
	var (
		start = time.Now()
		count atomic.Int64
		total atomic.Uint64
...
func printChainMetadata(db sildb.KeyValueStore) {
...
func ReadChainMetadata(db sildb.KeyValueStore) [][]string {
...
// deleted already.
func SafeDeleteRange(db sildb.KeyValueStore, start, end []byte, hashScheme bool, stopCallback func(bool) bool) error {
	if !hashScheme {
		...
		for {
			...
			case err == nil:
				return nil
			case errors.Is(err, sildb.ErrTooManyKeys):
