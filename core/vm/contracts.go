...
// PrecompiledContractsSilaHomestead contains the default set of pre-compiled Sila
// contracts used in the Frontier and SilaHomestead releases.
var PrecompiledContractsSilaHomestead = PrecompiledContracts{
...
// PrecompiledContractsSilaByzantium contains the default set of pre-compiled Sila
// contracts used in the SilaByzantium release.
var PrecompiledContractsSilaByzantium = PrecompiledContracts{
	common.BytesToAddress([]byte{0x1}): &ecrecover{},
	common.BytesToAddress([]byte{0x2}): &sha256hash{},
	common.BytesToAddress([]byte{0x3}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x4}): &dataCopy{},
	common.BytesToAddress([]byte{0x5}): &bigModExp{sip2565: false, sip7823: false, sip7883: false},
	common.BytesToAddress([]byte{0x6}): &bn256AddSilaByzantium{},
	common.BytesToAddress([]byte{0x7}): &bn256ScalarMulSilaByzantium{},
	common.BytesToAddress([]byte{0x8}): &bn256PairingSilaByzantium{},
}

// PrecompiledContractsSilaIstanbul contains the default set of pre-compiled Sila
// contracts used in the SilaIstanbul release.
var PrecompiledContractsSilaIstanbul = PrecompiledContracts{
	common.BytesToAddress([]byte{0x1}): &ecrecover{},
	common.BytesToAddress([]byte{0x2}): &sha256hash{},
	common.BytesToAddress([]byte{0x3}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x4}): &dataCopy{},
	common.BytesToAddress([]byte{0x5}): &bigModExp{sip2565: false, sip7823: false, sip7883: false},
	common.BytesToAddress([]byte{0x6}): &bn256AddSilaIstanbul{},
	common.BytesToAddress([]byte{0x7}): &bn256ScalarMulSilaIstanbul{},
	common.BytesToAddress([]byte{0x8}): &bn256PairingSilaIstanbul{},
	common.BytesToAddress([]byte{0x9}): &blake2F{},
}

// PrecompiledContractsSilaBerlin contains the default set of pre-compiled Sila
// contracts used in the SilaBerlin release.
var PrecompiledContractsSilaBerlin = PrecompiledContracts{
	common.BytesToAddress([]byte{0x1}): &ecrecover{},
	common.BytesToAddress([]byte{0x2}): &sha256hash{},
	common.BytesToAddress([]byte{0x3}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x4}): &dataCopy{},
...
// PrecompiledContractsSilaCancun contains the default set of pre-compiled Sila
// contracts used in the SilaCancun release.
var PrecompiledContractsSilaCancun = PrecompiledContracts{
	common.BytesToAddress([]byte{0x1}): &ecrecover{},
	common.BytesToAddress([]byte{0x2}): &sha256hash{},
	common.BytesToAddress([]byte{0x3}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x4}): &dataCopy{},
...
// PrecompiledContractsSilaPrague contains the set of pre-compiled Sila
// contracts used in the SilaPrague release.
var PrecompiledContractsSilaPrague = PrecompiledContracts{
	common.BytesToAddress([]byte{0x01}): &ecrecover{},
	common.BytesToAddress([]byte{0x02}): &sha256hash{},
	common.BytesToAddress([]byte{0x03}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x04}): &dataCopy{},
	common.BytesToAddress([]byte{0x05}): &bigModExp{sip2565: true, sip7823: false, sip7883: false},
	common.BytesToAddress([]byte{0x06}): &bn256AddSilaIstanbul{},
	common.BytesToAddress([]byte{0x07}): &bn256ScalarMulSilaIstanbul{},
	common.BytesToAddress([]byte{0x08}): &bn256PairingSilaIstanbul{},
...
var PrecompiledContractsBLS = PrecompiledContractsSilaPrague

var PrecompiledContractsVerkle = PrecompiledContractsSilaBerlin

// PrecompiledContractsSilaOsaka contains the set of pre-compiled Sila
// contracts used in the SilaOsaka release.
var PrecompiledContractsSilaOsaka = PrecompiledContracts{
	common.BytesToAddress([]byte{0x01}): &ecrecover{},
	common.BytesToAddress([]byte{0x02}): &sha256hash{},
	common.BytesToAddress([]byte{0x03}): &ripemd160hash{},
	common.BytesToAddress([]byte{0x04}): &dataCopy{},
...
var (
	PrecompiledAddressesSilaOsaka     []common.Address
	PrecompiledAddressesSilaPrague    []common.Address
	PrecompiledAddressesSilaCancun    []common.Address
	PrecompiledAddressesSilaBerlin    []common.Address
	PrecompiledAddressesSilaIstanbul  []common.Address
	PrecompiledAddressesSilaByzantium []common.Address
	PrecompiledAddressesSilaHomestead []common.Address
)

func init() {
	for k := range PrecompiledContractsSilaHomestead {
		PrecompiledAddressesSilaHomestead = append(PrecompiledAddressesSilaHomestead, k)
	}
	for k := range PrecompiledContractsSilaByzantium {
		PrecompiledAddressesSilaByzantium = append(PrecompiledAddressesSilaByzantium, k)
	}
	for k := range PrecompiledContractsSilaIstanbul {
		PrecompiledAddressesSilaIstanbul = append(PrecompiledAddressesSilaIstanbul, k)
	}
	for k := range PrecompiledContractsSilaBerlin {
		PrecompiledAddressesSilaBerlin = append(PrecompiledAddressesSilaBerlin, k)
	}
	for k := range PrecompiledContractsSilaCancun {
		PrecompiledAddressesSilaCancun = append(PrecompiledAddressesSilaCancun, k)
	}
	for k := range PrecompiledContractsSilaPrague {
		PrecompiledAddressesSilaPrague = append(PrecompiledAddressesSilaPrague, k)
	}
	for k := range PrecompiledContractsSilaOsaka {
		PrecompiledAddressesSilaOsaka = append(PrecompiledAddressesSilaOsaka, k)
	}
}
...
// berlinMultComplexity implements the multiplication complexity formula for SilaBerlin.
//
// def mult_complexity(x):
//
//	ceiling(x/8)^2
...
// osakaMultComplexity implements the multiplication complexity formula for SilaOsaka.
//
// For x <= 32: returns 16
...
// byzantiumModexpGas calculates the gas cost for the modexp precompile using SilaByzantium rules.
...
// berlinModexpGas calculates the gas cost for the modexp precompile using SilaBerlin rules.
...
// osakaModexpGas calculates the gas cost for the modexp precompile using SilaOsaka rules.
...
func (c *bigModExp) NormalizeInput(input []byte) ([]byte, bool) {
	...
	// whether that is an empty output or an SilaOsaka length failure.
...
// SilaByzantium and SilaIstanbul operations.
...
// bn256AddSilaIstanbul implements a native elliptic curve point addition conforming to
// SilaIstanbul consensus rules.
type bn256AddSilaIstanbul struct{}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *bn256AddSilaIstanbul) RequiredGas(input []byte) uint64 {
	return params.Bn256AddGasSilaIstanbul
}

func (c *bn256AddSilaIstanbul) Run(input []byte) ([]byte, error) {
	return runBn256Add(input)
}

func (c *bn256AddSilaIstanbul) Name() string {
	return "BN254_ADD"
}

func (c *bn256AddSilaIstanbul) Cacheable() bool { return true }

// NormalizeInput drops everything past the bytes runBn256Add reads.
func (c *bn256AddSilaIstanbul) NormalizeInput(input []byte) ([]byte, bool) {
	return normalizeZeroPadded(input, bn256AddInputLength), true
}

// bn256AddSilaByzantium implements a native elliptic curve point addition
// conforming to SilaByzantium consensus rules.
type bn256AddSilaByzantium struct{}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *bn256AddSilaByzantium) RequiredGas(input []byte) uint64 {
	return params.Bn256AddGasSilaByzantium
}

func (c *bn256AddSilaByzantium) Run(input []byte) ([]byte, error) {
	return runBn256Add(input)
}

func (c *bn256AddSilaByzantium) Name() string {
	return "BN254_ADD"
}

func (c *bn256AddSilaByzantium) Cacheable() bool { return true }

// NormalizeInput drops everything past the bytes runBn256Add reads.
func (c *bn256AddSilaByzantium) NormalizeInput(input []byte) ([]byte, bool) {
	return normalizeZeroPadded(input, bn256AddInputLength), true
}
...
// both SilaByzantium and SilaIstanbul operations.
...
// bn256ScalarMulSilaIstanbul implements a native elliptic curve scalar
// multiplication conforming to SilaIstanbul consensus rules.
type bn256ScalarMulSilaIstanbul struct{}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *bn256ScalarMulSilaIstanbul) RequiredGas(input []byte) uint64 {
	return params.Bn256ScalarMulGasSilaIstanbul
}

func (c *bn256ScalarMulSilaIstanbul) Run(input []byte) ([]byte, error) {
	return runBn256ScalarMul(input)
}

func (c *bn256ScalarMulSilaIstanbul) Name() string {
	return "BN254_MUL"
}

func (c *bn256ScalarMulSilaIstanbul) Cacheable() bool { return true }

// NormalizeInput drops everything past the bytes runBn256ScalarMul reads.
func (c *bn256ScalarMulSilaIstanbul) NormalizeInput(input []byte) ([]byte, bool) {
	return normalizeZeroPadded(input, bn256ScalarMulInputLength), true
}

// bn256ScalarMulSilaByzantium implements a native elliptic curve scalar
// multiplication conforming to SilaByzantium consensus rules.
type bn256ScalarMulSilaByzantium struct{}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *bn256ScalarMulSilaByzantium) RequiredGas(input []byte) uint64 {
	return params.Bn256ScalarMulGasSilaByzantium
}

func (c *bn256ScalarMulSilaByzantium) Run(input []byte) ([]byte, error) {
	return runBn256ScalarMul(input)
}

func (c *bn256ScalarMulSilaByzantium) Name() string {
	return "BN254_MUL"
}

func (c *bn256ScalarMulSilaByzantium) Cacheable() bool { return true }

// NormalizeInput drops everything past the bytes runBn256ScalarMul reads.
func (c *bn256ScalarMulSilaByzantium) NormalizeInput(input []byte) ([]byte, bool) {
	return normalizeZeroPadded(input, bn256ScalarMulInputLength), true
}
...
// bn256PairingSilaIstanbul implements a pairing pre-compile for the bn256 curve
// conforming to SilaIstanbul consensus rules.
type bn256PairingSilaIstanbul struct{}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *bn256PairingSilaIstanbul) RequiredGas(input []byte) uint64 {
	return params.Bn256PairingBaseGasSilaIstanbul + uint64(len(input)/192)*params.Bn256PairingPerPointGasSilaIstanbul
}

func (c *bn256PairingSilaIstanbul) Run(input []byte) ([]byte, error) {
	return runBn256Pairing(input)
}

func (c *bn256PairingSilaIstanbul) Name() string {
	return "BN254_PAIRING"
}

func (c *bn256PairingSilaIstanbul) Cacheable() bool { return true }

// NormalizeInput skips inputs Run rejects on length.
func (c *bn256PairingSilaIstanbul) NormalizeInput(input []byte) ([]byte, bool) {
	return input, len(input)%192 == 0
}

// bn256PairingSilaByzantium implements a pairing pre-compile for the bn256 curve
// conforming to SilaByzantium consensus rules.
type bn256PairingSilaByzantium struct{}

// RequiredGas returns the gas required to execute the pre-compiled contract.
func (c *bn256PairingSilaByzantium) RequiredGas(input []byte) uint64 {
	return params.Bn256PairingBaseGasSilaByzantium + uint64(len(input)/192)*params.Bn256PairingPerPointGasSilaByzantium
}

func (c *bn256PairingSilaByzantium) Run(input []byte) ([]byte, error) {
	return runBn256Pairing(input)
}

func (c *bn256PairingSilaByzantium) Name() string {
	return "BN254_PAIRING"
}

func (c *bn256PairingSilaByzantium) Cacheable() bool { return true }

// NormalizeInput skips inputs Run rejects on length.
func (c *bn256PairingSilaByzantium) NormalizeInput(input []byte) ([]byte, bool) {
	return input, len(input)%192 == 0
}
