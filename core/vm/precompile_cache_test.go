...
func TestPrecompileCacheNormalizationCollapsesPadding(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"bn256ScalarMul", &bn256ScalarMulSilaIstanbul{}, bn256ScalarMulInputLength, bn256ScalarMulInputLength},
	} {
...
func TestPrecompileCacheHit(t *testing.T) {
	var (
		...
		p     = PrecompiledContractsSilaOsaka[addr]
		rules = params.Rules{IsSilaBerlin: true, IsSilaIstanbul: true, IsSilaByzantium: true}
		input = make([]byte, 192)
	)
...
// sits at in allPrecompiles. The SilaByzantium bn256 variants share their Run with
// the SilaIstanbul ones and only differ in price, so they would be duplicate rows.
var precompileCacheFixtures = []struct{ fixture, addr string }{
	{"ecRecover", "01"}, {"modexp_sip2565", "f5"}, {"bn256Add", "06"},
...
func BenchmarkPrecompileCacheHitVsRun(b *testing.B) {
	rules := params.Rules{IsSilaByzantium: true, IsSilaIstanbul: true, IsSilaBerlin: true, IsSilaCancun: true, IsSilaPrague: true, IsSilaOsaka: true}
