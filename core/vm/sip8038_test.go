...
// Amsterdam harness from sip8037_test.go and assert the re-priced regular-gas,
...
func TestEIP8038SStore(t *testing.T) {
	...
	cases := []struct {
		...
		// Single store, cold access (SIP-8038 cases table, Cold rows + noop).
...
func TestEIP8038Calls(t *testing.T) {
	...
	// Calling an SIP-7702 authority accesses both the authority and its
