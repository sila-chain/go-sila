...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	var (
		...
		_, blocks, _ = GenerateChainWithGenesis(gspec, silash.NewFaker(), 8, nil)
	)
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), gspec, silash.NewFaker(), options)
	defer chain.Stop()
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				engine := silash.NewFakeFailer(headers[i].Number.Uint64())
				_, results = engine.VerifyHeaders(chain, []*types.Header{headers[i]})
			}
