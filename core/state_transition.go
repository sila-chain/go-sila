...
func intrinsicBaseGasSIP2780(from common.Address, to *common.Address, value *uint256.Int) uint64 {
	...
	// tx.to charge. Per SIP-2780 the recipient touch is charged at the cold
...
func FloorDataGas(rules params.Rules, from common.Address, to *common.Address, value *uint256.Int, data []byte, accessList types.AccessList) (uint64, error) {
	...
	// The floor is anchored to the transaction base cost. Under SIP-2780 that
...
// (plus `blobGas × blobBaseFee` under SilaCancun), the cap-vs-tip differential
...
func (st *stateTransition) preCheck(rules params.Rules) error {
	...
	if msg.BlobHashes != nil {
		...
		if rules.IsSilaOsaka && len(msg.BlobHashes) > params.BlobTxMaxBlobs {
	...
	// Check whether the init code size has been exceeded (SIP-3860).
	if msg.To == nil {
...
// chargeRuntimeGas deducts an SIP-2780 runtime charge from the transaction's
...
// entered:
//
//   - if the recipient is SIP-161 empty and the transaction carries value,
//     the durable state growth of the new account;
//
//   - if the recipient is an SIP-7702 delegated account, resolving the
...
// applyAuthorization applies an SIP-7702 code delegation to the state.
func (st *stateTransition) applyAuthorization(rules params.Rules, auth *types.SetCodeAuthorization, authorities map[common.Address]*authTracking) error {
	...
	if !rules.IsAmsterdam {
		...
		// SIP-2780: charge the state-dependent authorization costs at runtime.
...
// applyAuthorizations applies the SIP-7702 code delegations to the state.
