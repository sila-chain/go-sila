...
func IntrinsicGas(data []byte, accessList types.AccessList, authList []types.SetCodeAuthorization, from common.Address, to *common.Address, value *uint256.Int, rules params.Rules, costPerStateByte uint64) (vm.GasCosts, error) {
	...
	if rules.IsAmsterdam {
		gas.RegularGas = intrinsicBaseGasSIP2780(from, to, value)
		if isContractCreation {
			// New-account creation is charged as state gas (SIP-8037).
	...
	if dataLen > 0 {
		...
		if isContractCreation && rules.IsSilaShanghai {
...
// intrinsicBaseGasSIP2780 computes the regular-gas portion of the SIP-2780
...
type Message struct {
	...
	//
	// This field will be set to true for operations like RPC sil_call
...
//
// The balance requirement is the worst-case SIL the tx may need to lock
// up: `msg.GasLimit × max(msg.GasPrice, msg.GasFeeCap) + msg.Value`,
// plus `blobGas × msg.BlobGasFeeCap` under SilaCancun. Insufficient balance
...
// under SilaCancun), the cap-vs-tip differential is settled at tx end.
//
...
func (st *stateTransition) buyGas() error {
	...
	if st.evm.ChainConfig().IsSilaCancun(st.evm.Context.BlockNumber, st.evm.Context.Time) {
		if blobGas := st.blobGasUsed(); blobGas > 0 {
			...
			// BlobGasFeeCap. However, in sil_call it is still possible for users to specify
...
//   - SIP-1559 fee-cap, tip-cap and base-fee constraints (SilaLondon+).
//   - Blob-tx structural checks: non-nil `To`, non-empty hash list,
//     valid KZG versioned hashes, count below `BlobTxMaxBlobs` (SilaOsaka+).
//   - Blob fee-cap not below the current blob base fee (SilaCancun+).
//   - SIP-7702 set-code-tx shape: non-nil `To` and non-empty
//     authorization list.
//
// The SkipNonceChecks / SkipTransactionChecks / NoBaseFee flags bypass
// subsets of these checks for simulation paths (sil_call, sil_estimateGas).
func (st *stateTransition) preCheck() error {
	...
	var (
		isSilaOsaka = st.evm.ChainConfig().IsSilaOsaka(st.evm.Context.BlockNumber, st.evm.Context.Time)
		isAmsterdam = st.evm.ChainConfig().IsAmsterdam(st.evm.Context.BlockNumber, st.evm.Context.Time)
	)
	if !msg.SkipTransactionChecks {
		// Verify tx gas limit does not exceed SIP-7825 cap.
		if !isAmsterdam && isSilaOsaka && msg.GasLimit > params.MaxTxGas {
	...
	// Make sure that transaction gasFeeCap is greater than the baseFee (post sila_london)
	...
	if msg.BlobHashes != nil {
		...
		if isSilaOsaka && len(msg.BlobHashes) > params.BlobTxMaxBlobs {
...
func (st *stateTransition) execute() (*ExecutionResult, error) {
	...
	if contractCreation {
		...
		// SIP-2780: charge the transaction's top-level recipient costs. If the
		// budget cannot cover the charge, the top frame halts out of gas.
		if rules.IsAmsterdam && !st.chargeCallRecipientSIP2780(value) {
	...
	// Settle down the gas usage and refund the SIL back if any remaining
...
// chargeCallRecipientSIP2780 applies the SIP-2780 transaction top-level gas costs for
// a message-call transaction, charged before any opcode executes:
//
//   - if the recipient is SIP-161 non-existent and the transaction carries value,
//     charge for account creation.
//
//   - if the recipient is an SIP-7702 delegated account, resolving the delegation
...
func (st *stateTransition) chargeCallRecipientSIP2780(value *uint256.Int) bool {
	...
	// leave a transient SIP-161-empty account, no empty account can exist here, and
	...
	if _, ok := types.ParseDelegation(st.state.GetCode(to)); ok {
		// SIP-2780: The tx.sender, tx.to, and (where applicable) delegation-target
...
func (st *stateTransition) settleGas(rules params.Rules, floorDataGas uint64) (gasUsed, peakUsed uint64, err error) {
	...
	// Refund leftover gas to the sender as SIL.
	if gasLeft > 0 {
