...
func NewFilterMaps(db sildb.KeyValueStore, initView *ChainView, historyCutoff, finalBlock uint64, params Params, config Config) (*FilterMaps, error) {
...
func (f *FilterMaps) safeDeleteWithLogs(deleteFn func(db sildb.KeyValueStore, hashScheme bool, stopCb func(bool) bool) error, action string, stopCb func() bool) error {
	var (
		start          = time.Now()
		logPrinted     bool
...
func (f *FilterMaps) setRange(batch sildb.KeyValueWriter, newView *ChainView, newRange filterMapsRange, isTempRange bool) {
...
func (f *FilterMaps) storeFilterMapRows(batch sildb.Batch, mapIndices []uint32, rowIndex uint32, rows []FilterRow) error {
...
// belonging to the same base row group.
func (f *FilterMaps) storeFilterMapRowsOfGroup(batch sildb.Batch, mapIndices []uint32, rowIndex uint32, rows []FilterRow) error {
	var (
		baseRows    [][]uint32
		groupIndex  = f.mapGroupIndex(mapIndices[0])
		mapRowIndex = f.mapRowIndex(groupIndex, rowIndex)
...
// generated by the given block are located.
func (f *FilterMaps) storeBlockLvPointer(batch sildb.Batch, blockNumber, lvPointer uint64) {
	f.lvPointerCache.Add(blockNumber, lvPointer)
...
// generated by the given block are located.
func (f *FilterMaps) deleteBlockLvPointer(batch sildb.Batch, blockNumber uint64) {
...
func (f *FilterMaps) storeLastBlockOfMap(batch sildb.Batch, mapIndex uint32, number uint64, id common.Hash) {
	f.lastBlockCache.Add(mapIndex, lastBlockOfMap{number: number, id: id})
...
// log value entry of the given map.
func (f *FilterMaps) deleteLastBlockOfMap(batch sildb.Batch, mapIndex uint32) {
...
func (f *FilterMaps) deleteTailEpoch(epoch uint32) (bool, error) {
	...
	// remove index data
	deleteFn := func(db sildb.KeyValueStore, hashScheme bool, stopCb func(bool) bool) error {
