...
func TestSIP2780Gas(t *testing.T) {
	...
	cases := []struct {
		...
		// case 4: SIL transfer to an existing EOA.
		{"value/eoa", callTx(0, existingEOA, 1, 100_000, nil), base + cold + valueCst, 0},
		// case 5: SIL transfer to a contract.
		...
		// case 7: SIL transfer to a 7702-delegated account (no new-account charge).
...
// TestSIP2780NewAccountFunded verifies that a value transfer creating a new
// account both materializes and funds the recipient.
func TestSIP2780NewAccountFunded(t *testing.T) {
