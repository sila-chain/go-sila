...
func TestSIP8246SelfdestructNoBurn(t *testing.T) {
	...
	// on purpose: it would additionally verify the SIP-7928 block access list,
	...
	// SIP-8246: the self-destructed, freshly-created contract keeps its balance
	// rather than burning it, so the account survives.
	if got := state.GetBalance(created); got.ToBig().Cmp(value) != 0 {
		t.Errorf("created account balance = %v, want %v (SIP-8246: balance must be preserved, not burned)", got, value)
	}
