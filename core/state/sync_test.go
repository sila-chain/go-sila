...
func makeTestState(scheme string) (sildb.Database, Database, *triedb.Database, common.Hash, []*testAccount) {
...
// account array.
func checkStateAccounts(t *testing.T, db sildb.Database, scheme string, root common.Hash, accounts []*testAccount) {
...
func checkStateConsistency(db sildb.Database, scheme string, root common.Hash) error {
	config := &triedb.Config{Preimages: true}
	if scheme == rawdb.PathScheme {
		config.PathDB = pathdb.Defaults
	}
...
func copyPreimages(srcDb, dstDb sildb.Database) {
