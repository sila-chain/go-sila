...
type generatorContext struct {
	...
	batch   sildb.Batch         // Database batch for writing batch data atomically
	logged  time.Time           // The timestamp when last generation progress was displayed
}

// newGeneratorContext initializes the context for generation.
func newGeneratorContext(stats *generatorStats, db sildb.KeyValueStore, accMarker []byte, storageMarker []byte) *generatorContext {
...
func (ctx *generatorContext) removeStorageBefore(account common.Hash) {
	...
	for iter.Next() {
		...
		if ctx.batch.ValueSize() > sildb.IdealBatchSize {
			ctx.batch.Write()
			ctx.batch.Reset()
		}
	}
