...
func CheckDanglingStorage(chaindb sildb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb sildb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db sildb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db sildb.KeyValueStore, hash common.Hash) error {
