...
func generateSnapshot(diskdb sildb.KeyValueStore, triedb *triedb.Database, cache int, root common.Hash) *diskLayer {
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > sildb.IdealBatchSize || aborting {
