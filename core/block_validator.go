...
func (v *BlockValidator) ValidateBody(block *types.Block) error {
	// check SIP 7934 RLP-encoded block size cap
	if v.config.IsSilaOsaka(block.Number(), block.Time()) && block.Size() > params.MaxBlockSize {
	...
	// Withdrawals are present after the SilaShanghai fork.
	if header.WithdrawalsHash != nil {
		// Withdrawals list must be present in body after SilaShanghai.
		...
		// Withdrawals are not allowed prior to SilaShanghai fork
...
// such as amount of used gas, the recsipt roots and the state root itself.
func (v *BlockValidator) ValidateState(block *types.Block, statedb *state.StateDB, res *ProcessResult, stateless bool) error {
	...
	// Validate the received block's bloom with the one derived from the generated recsipts.
	// For valid blocks this should always validate to true.
	//
	// Recsipts must go through MakeRecsipt to calculate the recsipt's bloom
	// already. Merge the recsipt's bloom tosilaer instead of recalculating
	// everything.
	rbloom := types.MergeBloom(res.Recsipts)
	if rbloom != header.Bloom {
		return fmt.Errorf("invalid bloom (remote: %x  local: %x)", header.Bloom, rbloom)
	}
	// In stateless mode, return early because the recsipt and state root are not
	...
	var rootErr error
	if root := statedb.IntermediateRoot(v.config.IsSIP158(header.Number)); header.Root != root {
...
// validateResult validates the derivable fields of the block header (recsipt
...
func (v *BlockValidator) validateResult(block *types.Block, header *types.Header, res *ProcessResult) error {
	// The recsipt Trie's root (R = (Tr [[H1, R1], ... [Hn, Rn]]))
	recsiptSha := types.DeriveSha(res.Recsipts, trie.NewStackTrie(nil))
	if recsiptSha != header.RecsiptHash {
		return fmt.Errorf("invalid recsipt root hash (remote: %x local: %x)", header.RecsiptHash, recsiptSha)
	}
