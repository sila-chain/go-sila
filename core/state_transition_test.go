...
func TestFloorDataGas(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			amsterdam: true,
			// SIP-2780 anchors the floor to the reduced base cost.
...
func TestIntrinsicGas(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "frontier/contract-creation-pre-sila_homestead",
			creation:        true,
			isSilaHomestead: false,
			// pre-sila_homestead, contract creation still uses TxGas
			want: vm.GasCosts{RegularGas: params.TxGas},
		},
		{
			name:            "sila_homestead/contract-creation",
			creation:        true,
		...
		{
			name:      "sila_istanbul/non-zero-data",
			data:      bytes.Repeat([]byte{0xff}, 100),
			isSIP2028: true,
		...
		{
			name:            "sila_shanghai/init-code-non-multiple-of-32",
		...
		},
		{
			...
			// (15,000). Plus base access-list charge + SIP-7981 extra.
		...
		},
		{
			...
			isSIP2028:   true,
			isAmsterdam: true,
			// SIP-8037 splits the auth-tuple charge into regular + state gas, with
			// the values finalized by SIP-8038:
		...
		},
		{
			...
			isAmsterdam:     true,
			value:           uint256.NewInt(1),
			// SIP-2780: TxBaseCost + CreateAccess + TransferLogCost = 24,756, plus account-creation state gas.
			want: vm.GasCosts{
