...
type blobTxMeta struct {
	...
	sizeWithoutBlob uint64 // RLP-encoded size of transaction without blob data (for SIL/72)
...
// txSizeWithoutBlob returns the sil/72 network size, where the blob payload is dropped
...
// transaction encoding for the given sil protocol version. Used for getRLP.
//
// Stored RLP: [type_byte || tx_fields, [version, [cells], [comms], [proofs], custody]]
//
// sil/69, sil/70: [blobs] is recovered from stored cells via kzg.
//
//	V0: type_byte || rlp([tx_fields, [blobs], [comms], [proofs]])
//	V1: type_byte || rlp([tx_fields, version, [blobs], [comms], [proofs]])
//
// sil/72: [blobs] is replaced by an empty list (cells are fetched separately
//
//	      via GetCells).
...
func encodeForNetwork(storedRLP []byte, version uint) ([]byte, error) {
	...
	var blobsField []byte
	// todo - Didn't use sil.SIL72 due to circular import error in test
	if version >= 72 {
		// sil/72 omits the blob payload; peers fetch cells separately via GetCells.
		blobsField = []byte{0xc0} // RLP-encoded empty list
	} else {
		// sil/69, sil/70 need actual blobs: recover them from stored cells.
...
func (p *BlobPool) Init(gasTip uint64, head *types.Header, reserver txpool.Reserver) error {
	...
	var (
		basefee = uint256.MustFromBig(sip1559.CalcBaseFee(p.chain.Config(), head))
...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	if p.chain.Config().IsSilaCancun(newHead.Number, newHead.Time) {
		p.limbo.finalize(p.chain.CurrentFinalBlock())
	}
	// Reset the price heap for the new set of basefee/blobfee pairs
	var (
		basefee = uint256.MustFromBig(sip1559.CalcBaseFee(p.chain.Config(), newHead))
		blobfee = uint256.MustFromBig(big.NewInt(params.BlobTxMinBlobGasprice))
	)
	if newHead.ExcessBlobGas != nil {
		blobfee = uint256.MustFromBig(sip4844.CalcBlobFee(p.chain.Config(), newHead))
	}
...
func (p *BlobPool) reinject(addr common.Address, txhash common.Hash) error {
	...
	// Post-SilaOsaka, legacy (v0) blob sidecars are no longer accepted into the pool.
	// A reorged-out legacy blob transaction can therefore not be re-added, so drop
	// it on the floor instead of putting it back.
	head := p.head.Load()
	if p.chain.Config().IsSilaOsaka(head.Number, head.Time) && ptx.CellSidecar.Version == types.BlobSidecarVersion0 {
...
func (p *BlobPool) updateStorageMetrics() {
	...
	for _, shelf := range stats.Shelves {
		...
		maxBlobs := sip4844.LatestMaxBlobsPerBlock(p.chain.Config())
