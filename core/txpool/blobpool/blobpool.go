...
//     commit to their own current state, which is independent of Sila sila-mainnet
...
func (p *BlobPool) reinject(addr common.Address, txhash common.Hash) error {
	...
	head := p.head.Load()
	if p.chain.Config().IsSilaOsaka(head.Number, head.Time) && ptx.Version == types.BlobSidecarVersion0 {
		log.Debug("Dropping reorged legacy blob transaction", "hash", txhash)
		return errors.New("legacy blob sidecar unsupported post-sila_osaka")
	}
	blob, err := rlp.EncodeToBytes(ptx)
	if err != nil {
...
// It converts the pool's internal type to the RLP format used by the sil protocol:
