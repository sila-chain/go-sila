...
func (bc *testBlockChain) GetBlock(hash common.Hash, number uint64) *types.Block {
	...
	if number == bc.config.SilaLondonBlock.Uint64()+1 {
...
// encoding for each (sidecar version, sil protocol version) combination.
//   - sil/69, sil/70: blobs recovered from cells, output matches the original tx
//   - sil/72:         blob payload omitted, output matches removeBlobs(tx)
func TestEncodeForNetwork(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testEncodeForNetwork(t, tc.silVer)
		})
	}
}

func testEncodeForNetwork(t *testing.T, silVer uint) {
	key, _ := crypto.GenerateKey()
	...
	gotRLP, err := encodeForNetwork(storedRLP, silVer)
	if err != nil {
