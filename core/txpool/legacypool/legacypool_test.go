...
var (
	...
	// sip1559Config is a chain config with SIP-1559 enabled at block 0.
	sip1559Config *params.ChainConfig
)
...
func init() {
	...
	sip1559Config = &cpy
	sip1559Config.SilaBerlinBlock = common.Big0
	sip1559Config.SilaLondonBlock = common.Big0
}
...
func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.SilaHomesteadSigner{}, key)
	return tx
}
...
func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey, dataBytes uint64) *types.Transaction {
	...
	for i := 0; i < retryTimes; i++ {
		...
		tx, _ = types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(0), gaslimit, gasprice, data), types.SilaHomesteadSigner{}, key)
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.SilaHomesteadSigner{}, tx)
}
...
func TestNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.SilaHomesteadSigner{}, key)
	from, _ := deriveSender(tx)
	testAddBalance(pool, from, big.NewInt(1))
...
func TestTipAboveFeeCap(t *testing.T) {
	...
	pool, key := setupPoolWithConfig(sip1559Config)
	defer pool.Close()
...
func TestMinGasPriceEnforced(t *testing.T) {
	...
	// Create the pool to test the pricing enforcement with
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabaseForTesting())
	blockchain := newTestBlockChain(sip1559Config, 10000000, statedb, new(event.Feed))
...
func TestRepricingDynamicFee(t *testing.T) {
	...
	// Create the pool to test the pricing enforcement with
	pool, _ := setupPoolWithConfig(sip1559Config)
	defer pool.Close()
