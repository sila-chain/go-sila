...
func (pool *LegacyPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*SortedMap) {
	...
	if reset != nil {
		if reset.newHead != nil && reset.oldHead != nil {
			...
			// SilaOsaka fork boundary.
			if pool.chainconfig.IsSilaOsaka(reset.newHead.Number, reset.newHead.Time) &&
				!pool.chainconfig.IsSilaOsaka(reset.oldHead.Number, reset.oldHead.Time) {
	...
	if reset != nil {
		...
		if reset.newHead != nil {
			if pool.chainconfig.IsSilaLondon(new(big.Int).Add(reset.newHead.Number, big.NewInt(1))) {
