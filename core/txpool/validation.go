...
func ValidateTransaction(tx *types.Transaction, head *types.Header, signer types.Signer, opts *ValidationOptions) error {
	...
	if !rules.IsSilaBerlin && tx.Type() != types.LegacyTxType {
		return fmt.Errorf("%w: type %d rejected, pool not yet in SilaBerlin", core.ErrTxTypeNotSupported, tx.Type())
	}
	if !rules.IsSilaLondon && tx.Type() == types.DynamicFeeTxType {
		return fmt.Errorf("%w: type %d rejected, pool not yet in SilaLondon", core.ErrTxTypeNotSupported, tx.Type())
	}
	if !rules.IsSilaCancun && tx.Type() == types.BlobTxType {
		return fmt.Errorf("%w: type %d rejected, pool not yet in SilaCancun", core.ErrTxTypeNotSupported, tx.Type())
	}
	if !rules.IsSilaPrague && tx.Type() == types.SetCodeTxType {
		return fmt.Errorf("%w: type %d rejected, pool not yet in SilaPrague", core.ErrTxTypeNotSupported, tx.Type())
	}
...
func ValidateCells(sidecar *types.BlobTxCellSidecar) error {
	...
	// However it is required to 1) serve sil71 peer and direct submission 2) catch any bug in
	...
	return validateCellsSilaOsaka(sidecar)
}

func validateCellsSilaOsaka(sidecar *types.BlobTxCellSidecar) error {
