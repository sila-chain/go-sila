...
func newGQLService(t *testing.T, stack *node.Node, sila_shanghai bool, gspec *core.Genesis, genBlocks int, genfunc func(i int, gen *core.BlockGen)) (*handler, []*types.Block) {
	...
	if sila_shanghai {
		...
		// SilaShanghai upgrade at block 1.
		shanghaiTime := uint64(5)
		gspec.Config.SilaShanghaiTime = &shanghaiTime
	}

	silBackend, err := sil.New(stack, silConf)
	if err != nil {
		t.Fatalf("could not create sil backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AllSilashProtocolChanges, silBackend.BlockChain().Genesis(),
		engine, silBackend.ChainDb(), genBlocks, genfunc)
	_, err = silBackend.BlockChain().InsertChain(chain)
	if err != nil {
		t.Fatalf("could not create import blocks: %v", err)
	}
	// Set up handler
	filterSystem := filters.NewFilterSystem(silBackend.APIBackend, filters.Config{})
	handler, err := newHandler(stack, silBackend.APIBackend, filterSystem, []string{}, []string{})
	if err != nil {
		t.Fatalf("could not create graphql service: %v", err)
	}
