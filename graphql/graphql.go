...
// Package graphql provides a GraphQL interface to Sila node data.
package graphql
...
// Account represents an Sila account at a particular block.
type Account struct {
	r             *Resolver
...
// Transaction represents an Sila transaction.
...
func (b *Block) NextBaseFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	...
	if header.BaseFee == nil {
		...
		if !chaincfg.IsSilaLondon(new(big.Int).Add(header.Number, common.Big1)) {
			return nil, nil
		}
	}
...
type CallData struct {
	From                 *common.Address // The Sila address the call is from.
	To                   *common.Address // The Sila address the call is to.
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	return silapi.DoEstimateGas(ctx, b.r.backend, args.Data, *b.numberOrHash, nil, nil, b.r.backend.RPCGasCap())
}
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	return silapi.DoEstimateGas(ctx, p.r.backend, args.Data, latestBlockNr, nil, nil, p.r.backend.RPCGasCap())
}

// Resolver is the top-level object in the GraphQL hierarchy.
type Resolver struct {
	backend      silapi.Backend
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := silapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
