...
func GetChainConfig(forkString string) (baseConfig *params.ChainConfig, sips []int, err error) {
	...
	for _, sip := range eipsStrings {
		if eipNum, err := strconv.Atoi(sip); err != nil {
			return nil, nil, fmt.Errorf("syntax error, invalid sip number %v", sip)
		} else {
			if !vm.ValidSip(eipNum) {
				return nil, nil, fmt.Errorf("syntax error, invalid sip number %v", eipNum)
			}
...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool, scheme string) (st StateTestState, root common.Hash, gasUsed uint64, err error) {
	...
	var baseFee *big.Int
	if config.IsSilaLondon(new(big.Int)) {
	...
	if config.IsSilaCancun(new(big.Int), block.Time()) {
	...
	if config.IsSilaCancun(new(big.Int), block.Time()) && t.json.Env.ExcessBlobGas != nil {
		header := &types.Header{
			Time:          block.Time(),
...
func MakePreState(db sildb.Database, accounts types.GenesisAlloc, snapshotter bool, scheme string) StateTestState {
