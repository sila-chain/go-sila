...
var (
	...
	// parent block's time and difficulty. The calculation uses the SilaByzantium rules, but with
	// bomb offset 5M.
	// Specification SIP-1234: https://sips.sila.org/SIPS/sip-1234
	calcDifficultySilaConstantinople = makeDifficultyCalculator(big.NewInt(5000000))

	// calcDifficultySilaByzantium is the difficulty adjustment algorithm. It returns
	// the difficulty that a new block should have when created at time given the
	// parent block's time and difficulty. The calculation uses the SilaByzantium rules.
	// Specification SIP-649: https://sips.sila.org/SIPS/sip-649
	calcDifficultySilaByzantium = makeDifficultyCalculator(big.NewInt(3000000))
)
...
func (silash *Silash) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules of the
// stock Sila silash engine.
func (silash *Silash) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	return silash.verifyHeader(chain, header, parent, false, time.Now().Unix())
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers
// concurrently. The method returns a quit channel to abort the operations and
// a results channel to retrieve the async verifications.
func (silash *Silash) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if silash.fakeFull || len(headers) == 0 {
	...
	go func() {
		for i, header := range headers {
			...
			if parent == nil {
				...
				err = silash.verifyHeader(chain, header, parent, false, unixNow)
			}
			select {
			case <-abort:
				return
...
// rules of the stock Sila silash engine.
func (silash *Silash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	// If we're running a full engine faking, accept any input as valid
	if silash.fakeFull {
		return nil
	}
	...
	for _, uncle := range block.Uncles() {
		...
		if err := silash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, time.Now().Unix()); err != nil {
			return err
		}
	}
	return nil
...
func (silash *Silash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, unixNow int64) error {
	...
	expected := silash.CalcDifficulty(chain, header.Time, parent)
	...
	if !chain.Config().IsSilaLondon(header.Number) {
	...
	if silash.fakeDelay != nil {
		time.Sleep(*silash.fakeDelay)
	}
	if silash.fakeFail != nil && *silash.fakeFail == header.Number.Uint64() {
...
func (silash *Silash) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	case config.IsSilaByzantium(next):
		return calcDifficultySilaByzantium(time, parent)
	case config.IsSilaHomestead(next):
		return calcDifficultySilaHomestead(time, parent)
	default:
...
// the difficulty is calculated with SilaByzantium rules, which differs from SilaHomestead in
...
func makeDifficultyCalculator(bombDelay *big.Int) func(time uint64, parent *types.Header) *big.Int {
	...
	return func(time uint64, parent *types.Header) *big.Int {
		// https://github.com/sila-chain/SIPs/issues/100.
		// algorithm:
		// diff = (parent_diff +
...
// calcDifficultySilaHomestead is the difficulty adjustment algorithm. It returns
// the difficulty that a new block should have when created at time given the
...
var SilaHomesteadDifficultyCalculator = calcDifficultySilaHomestead
var DynamicDifficultyCalculator = makeDifficultyCalculator

// Prepare implements consensus.Engine, initializing the difficulty field of a
// header to conform to the silash protocol. The changes are done inline.
func (silash *Silash) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	parent := chain.GetHeader(header.ParentHash, header.Number.Uint64()-1)
	if parent == nil {
		return consensus.ErrUnknownAncestor
	}
	header.Difficulty = silash.CalcDifficulty(chain, header.Time, parent)
	return nil
}
...
func (silash *Silash) SealHash(header *types.Header) (hash common.Hash) {
	...
	if header.WithdrawalsHash != nil {
		panic("withdrawal hash set on silash")
	}
	if header.ExcessBlobGas != nil {
		panic("excess blob gas set on silash")
	}
	if header.BlobGasUsed != nil {
		panic("blob gas used set on silash")
	}
	if header.ParentBeaconRoot != nil {
		panic("parent beacon root set on silash")
	}
	if header.SlotNumber != nil {
		panic("slot number set on silash")
	}
...
func accumulateRewards(config *params.ChainConfig, stateDB vm.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if config.IsSilaByzantium(header.Number) {
		blockReward = SilaByzantiumBlockReward
	}
	if config.IsSilaConstantinople(header.Number) {
		blockReward = SilaConstantinopleBlockReward
	}
