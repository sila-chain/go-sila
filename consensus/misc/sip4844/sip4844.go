...
func CalcExcessBlobGas(config *params.ChainConfig, parent *types.Header, headTimestamp uint64) uint64 {
	isSilaOsaka := config.IsSilaOsaka(config.SilaLondonBlock, headTimestamp)
	bcfg, err := latestBlobConfig(config, headTimestamp)
	if err != nil {
		panic("calculating excess blob gas on nil blob config")
	}
	return calcExcessBlobGas(isSilaOsaka, bcfg, parent)
}

func calcExcessBlobGas(isSilaOsaka bool, bcfg BlobConfig, parent *types.Header) uint64 {
	...
	// SIP-7918 (post-SilaOsaka) introduces a different formula for computing excess,
