...
func TestCalcExcessBlobGas(t *testing.T) {
	var (
		...
		targetBlobs   = config.BlobScheduleConfig.SilaCancun.Target
	...
	for i, tt := range tests {
		...
		result := CalcExcessBlobGas(config, header, *config.SilaCancunTime)
		if result != tt.want {
...
func TestCalcBlobFee(t *testing.T) {
	...
	for i, tt := range tests {
		config := &params.ChainConfig{SilaLondonBlock: big.NewInt(0), SilaCancunTime: &zero, BlobScheduleConfig: params.DefaultBlobSchedule}
...
func TestCalcBlobFeePostSilaOsaka(t *testing.T) {
	zero := uint64(0)
...
func TestCalcExcessBlobGasEIP7918(t *testing.T) {
	...
	for _, tc := range tests {
		got := CalcExcessBlobGas(cfg, tc.header, *cfg.SilaCancunTime)
