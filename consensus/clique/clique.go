...
// ecrecover extracts the Sila account address from a signed header.
func ecrecover(header *types.Header, sigcache *sigLRU) (common.Address, error) {
	...
	// Recover the public key and the Sila address
...
type Clique struct {
	...
	db     sildb.Database       // Database to store and retrieve snapshot checkpoints
	...
	signer common.Address // Sila address of the signing key
...
func New(config *params.CliqueConfig, db sildb.Database) *Clique {
...
// Author implements consensus.Engine, returning the Sila address recovered
