...
func (at *authTest) Run(t *testing.T) {
	...
	err = cl.CallContext(ctx, &x, "sil_helloWorld")
...
func TestAuthEndpoints(t *testing.T) {
	...
	// Sila must read it from a file, and does not support in-memory JWT secrets, so we create a temporary file.
	...
	conf := &Config{
		...
		WSModules:   []string{"sil", "engine"},
		HTTPModules: []string{"sil", "engine"},
	}
	node, err := New(conf)
	if err != nil {
