...
func (n *Node) OpenDatabaseWithOptions(name string, opt DatabaseOptions) (sildb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
	}
	var db sildb.Database
	var err error
	if n.config.DataDir == "" {
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (sildb.Database, error) {
...
// If the node has no data directory, an in-memory database is returned.
// Deprecated: use OpenDatabaseWithOptions instead.
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (sildb.Database, error) {
	return n.OpenDatabaseWithOptions(name, DatabaseOptions{
...
func (n *Node) wrapDatabase(db sildb.Database) sildb.Database {
