...
type Config struct {
	...
	// AllowUnprotectedTxs allows non SIP-155 protected transactions to be send over RPC.
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Sila", keep that.
	if name == "sila" || name == "sila-testnet" {
		name = "Sila"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
