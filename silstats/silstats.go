...
// Package silstats implements the network stats reporting service.
package silstats
...
// backend encompasses the bare-minimum functionality needed for silstats reporting
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(silstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["sil"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ethproto.NodeInfo).Network)
	} else {
		return errors.New("no sil protocol available")
	}
	auth := &authMsg{
		ID: s.node,
		Info: nodeInfo{
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the silstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to silstats", "latency", latency)
...
func (s *Service) reportNewPayload(conn *connWrapper, ev core.NewPayloadEvent) error {
	...
	log.Trace("Sending new payload to silstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportBlock(conn *connWrapper, header *types.Header) error {
	...
	log.Trace("Sending new block to silstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to silstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to silstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to silstats")
