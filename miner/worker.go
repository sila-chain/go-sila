...
import (
	...
	"github.com/sila-chain/go-sila/core/txpool/txorder"
...
func (miner *Miner) generateWork(ctx context.Context, genParam *generateParams, witness bool) (result *newPayloadResult) {
	...
	// if SilaShanghai has been activated.
	body := types.Body{
		Transactions: work.txs,
		Withdrawals:  genParam.withdrawals,
	}
	if !miner.chainConfig.IsSilaShanghai(work.header.Number, work.header.Time) {
...
func (miner *Miner) prepareWork(ctx context.Context, genParams *generateParams, witness bool) (result *environment, err error) {
	...
	if miner.chainConfig.IsSilaLondon(header.Number) {
		header.BaseFee = sip1559.CalcBaseFee(miner.chainConfig, parent)
		if !miner.chainConfig.IsSilaLondon(parent.Number) {
	...
	if miner.chainConfig.IsSilaCancun(header.Number, header.Time) {
		var excessBlobGas uint64
		if miner.chainConfig.IsSilaCancun(parent.Number, parent.Time) {
...
func (miner *Miner) fillTransactions(ctx context.Context, interrupt *atomic.Int32, env *environment) (err error) {
	...
	filter.BlobTxs = true
	if miner.chainConfig.IsSilaOsaka(env.header.Number, env.header.Time) {
