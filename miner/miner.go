...
// Package miner implements Sila block creation and mining.
package miner
...
var DefaultConfig = Config{
	...
	// for payload generation. It should be enough for Sila to
	// run 3 rounds.
...
func New(sil Backend, config Config, engine consensus.Engine) *Miner {
	return &Miner{
		config:      &config,
		chainConfig: sil.BlockChain().Config(),
		engine:      engine,
		txpool:      sil.TxPool(),
