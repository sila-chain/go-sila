...
func makeNode(genesis *core.Genesis) (*node.Node, *sil.Sila, *catalyst.SimulatedBeacon, error) {
	...
	// Start the node and configure a full Sila node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, nil, nil, err
	}
	// Create and register the backend
	silBackend, err := sil.New(stack, &silconfig.Config{
		Genesis:            genesis,
		...
		GPO:                silconfig.Defaults.GPO,
		Miner:              silconfig.Defaults.Miner,
	...
	driver, err := catalyst.NewSimulatedBeacon(0, common.Address{}, silBackend)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	return stack, silBackend, driver, nil
}
