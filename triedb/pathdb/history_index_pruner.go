...
func newIndexPruner(disk sildb.KeyValueStore, typ historyType) *indexPruner {
	p := &indexPruner{
		disk:     disk,
		typ:      typ,
...
func (p *indexPruner) prunePrefix(prefix []byte, elemType elementType, tail uint64) (int, error) {
	var (
		...
		batch  = p.disk.NewBatchWithSize(sildb.IdealBatchSize)
	)
	for {
...
func (p *indexPruner) pruneEntry(batch sildb.Batch, ident stateIdent, blob []byte, bsize int, tail uint64) (int, error) {
