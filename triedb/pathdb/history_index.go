...
func loadIndexData(db sildb.KeyValueReader, state stateIdent, bitmapSize int) ([]*indexBlockDesc, error) {
...
func newIndexReader(db sildb.KeyValueReader, state stateIdent, bitmapSize int) (*indexReader, error) {
...
// indexing.
func newIndexWriter(db sildb.KeyValueReader, state stateIdent, limit uint64, bitmapSize int) (*indexWriter, error) {
	blob := readStateIndex(state, db)
	if len(blob) == 0 {
...
func (w *indexWriter) finish(batch sildb.Batch) {
	var (
...
func newIndexDeleter(db sildb.KeyValueReader, state stateIdent, limit uint64, bitmapSize int) (*indexDeleter, error) {
	blob := readStateIndex(state, db)
	if len(blob) == 0 {
...
//
// This function is safe to be called multiple times.
func (d *indexDeleter) finish(batch sildb.Batch) {
...
func readStateIndex(ident stateIdent, db sildb.KeyValueReader) []byte {
	switch ident.typ {
	case typeAccount:
...
func writeStateIndex(ident stateIdent, db sildb.KeyValueWriter, data []byte) {
	switch ident.typ {
	case typeAccount:
...
// This function is shared by accounts and storage slots.
func deleteStateIndex(ident stateIdent, db sildb.KeyValueWriter) {
	switch ident.typ {
	case typeAccount:
...
func readStateIndexBlock(ident stateIdent, db sildb.KeyValueReader, id uint32) []byte {
	switch ident.typ {
	case typeAccount:
...
// given state identifier. This function is shared by accounts and storage slots.
func writeStateIndexBlock(ident stateIdent, db sildb.KeyValueWriter, id uint32, data []byte) {
	switch ident.typ {
	case typeAccount:
...
func deleteStateIndexBlock(ident stateIdent, db sildb.KeyValueWriter, id uint32) {
	switch ident.typ {
	case typeAccount:
