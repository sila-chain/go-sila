...
type stateIdent struct {
	...
	// because, in legacy state histories (prior to the SilaCancun fork), the slot
...
func truncateFromHead(store sildb.AncientStore, typ historyType, nhead uint64) (int, error) {
...
// on the given parameters. It returns the number of items that were removed.
func truncateFromTail(store sildb.AncientStore, typ historyType, ntail uint64) (int, error) {
	ohead, err := store.Ancients()
	if err != nil {
		return 0, err
	}
...
func purgeHistory(store sildb.ResettableAncientStore, disk sildb.KeyValueStore, typ historyType) {
	if store == nil {
		return
	}
...
func syncHistory(stores ...sildb.AncientWriter) error {
...
// unexpected panics.
func repairHistory(db sildb.Database, isUBT bool, readOnly bool, stateID uint64, enableTrienode bool) (sildb.ResettableAncientStore, sildb.ResettableAncientStore, error) {
	...
	var trienodes sildb.ResettableAncientStore
	if enableTrienode {
	...
	truncate := func(store sildb.AncientStore, typ historyType, nhead uint64) {
		if store == nil {
			return
		}
