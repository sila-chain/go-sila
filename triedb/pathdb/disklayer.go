...
func (dl *diskLayer) writeHistory(typ historyType, diff *diffLayer) (bool, error) {
	var (
		...
		writeFunc func(writer sildb.AncientWriter, dl *diffLayer) error
	)
	switch typ {
	...
	case typeTrienodeHistory:
		...
		writeFunc = func(writer sildb.AncientWriter, diff *diffLayer) error {
...
func (dl *diskLayer) commit(bottom *diffLayer, force bool) (*diskLayer, error) {
	...
	if combined.full() || force || flush {
		...
		dl.frozen = combined
		dl.frozen.flush(bottom.root, dl.db.diskdb, []sildb.AncientWriter{dl.db.stateFreezer, dl.db.trienodeFreezer}, progress, dl.nodes, dl.states, bottom.stateID(), func() {
