...
type diskReader struct{ db sildb.KeyValueStore }
...
type generator struct {
	...
	db    sildb.KeyValueStore // Key-value store containing the snapshot data
...
func newGenerator(db sildb.KeyValueStore, noBuild bool, progress []byte, stats *generatorStats) *generator {
	if stats == nil {
...
func journalProgress(db sildb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (g *generator) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > sildb.IdealBatchSize || abort != nil {
