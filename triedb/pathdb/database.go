...
type Database struct {
	...
	diskdb sildb.Database // Persistent storage for matured trie nodes
	tree   *layerTree     // The group for all known layers

	stateFreezer sildb.ResettableAncientStore // Freezer for storing state histories, nil possible in tests
	stateIndexer *historyIndexer              // History indexer historical state data, nil possible

	trienodeFreezer sildb.ResettableAncientStore // Freezer for storing trienode histories, nil possible in tests
...
func New(diskdb sildb.Database, config *Config, isUBT bool) *Database {
	if config == nil {
		config = Defaults
	}
