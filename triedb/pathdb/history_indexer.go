...
func loadIndexMetadata(db sildb.KeyValueReader, typ historyType) *indexMetadata {
	var blob []byte
	switch typ {
	case typeStateHistory:
...
func storeIndexMetadata(db sildb.KeyValueWriter, typ historyType, last uint64) {
	m := indexMetadata{
...
func deleteIndexMetadata(db sildb.KeyValueWriter, typ historyType) {
	switch typ {
	case typeStateHistory:
...
type batchIndexer struct {
	...
	db      sildb.KeyValueStore       // Key-value database used to store or delete index data.
}

// newBatchIndexer constructs the batch indexer with the supplied mode.
func newBatchIndexer(db sildb.KeyValueStore, delete bool, typ historyType) *batchIndexer {
	return &batchIndexer{
...
func (b *batchIndexer) finish(force bool) error {
	...
	var (
		...
		batch     = b.db.NewBatchWithSize(sildb.IdealBatchSize)
		batchSize int
		batchMu   sync.RWMutex

		writeBatch = func(fn func(batch sildb.Batch)) error {
			batchMu.Lock()
	...
	for ident, list := range b.index {
		...
		eg.Go(func() error {
			if !b.delete {
				...
				return writeBatch(func(batch sildb.Batch) {
					iw.finish(batch)
				})
			} else {
...
func indexSingle(historyID uint64, db sildb.KeyValueStore, freezer sildb.AncientReader, typ historyType) error {
	start := time.Now()
	defer func() {
...
func unindexSingle(historyID uint64, db sildb.KeyValueStore, freezer sildb.AncientReader, typ historyType) error {
	start := time.Now()
	defer func() {
		if typ == typeStateHistory {
...
func newIndexIniter(disk sildb.Database, freezer sildb.AncientStore, typ historyType, lastID uint64, noWait bool) *indexIniter {
	initer := &indexIniter{
...
type historyIndexer struct {
	...
	pruner  *indexPruner
	typ     historyType
	disk    sildb.KeyValueStore
	freezer sildb.AncientStore
}

// checkVersion checks whether the index data in the database matches the version.
func checkVersion(disk sildb.KeyValueStore, typ historyType) {
	var blob []byte
	if typ == typeStateHistory {
...
func newHistoryIndexer(disk sildb.Database, freezer sildb.AncientStore, lastHistoryID uint64, typ historyType, noWait bool) *historyIndexer {
