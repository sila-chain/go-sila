...
func sanitizeRange(start, end uint64, freezer sildb.AncientReader) (uint64, uint64, error) {
...
func inspectHistory(freezer sildb.AncientReader, start, end uint64, onHistory func(*stateHistory, *HistoryStats)) (*HistoryStats, error) {
	var (
		stats  = &HistoryStats{}
		init   = time.Now()
		logged = time.Now()
...
func accountHistory(freezer sildb.AncientReader, address common.Address, start, end uint64) (*HistoryStats, error) {
...
func storageHistory(freezer sildb.AncientReader, address common.Address, slot common.Hash, start uint64, end uint64) (*HistoryStats, error) {
...
func historyRange(freezer sildb.AncientReader) (uint64, uint64, error) {
	// Load the id of the first history object in local store.
	tail, err := freezer.Tail(rawdb.DefaultHistoryGroup)
	if err != nil {
		return 0, 0, err
