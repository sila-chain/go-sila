...
func (s *nodeSet) revertTo(db sildb.KeyValueReader, nodes map[common.Hash]map[string]*trienode.Node) {
	var delta int64
	for owner, subset := range nodes {
		if owner == (common.Hash{}) {
			// Account trie nodes
...
func (s *nodeSet) write(batch sildb.Batch, clean *fastcache.Cache) int {
...
func encodeNodeCompressed(addExtension bool, elements [][]byte, indices []int) []byte {
	...
	if !addExtension {
		...
		for _, pos := range indices {
			...
			// in practice this field is never used in the Sila case. If it occurs,
