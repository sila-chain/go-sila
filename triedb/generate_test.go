...
func assertCanonicalNodes(t *testing.T, db sildb.Database, accounts []testAccount) {
	t.Helper()
...
func diskNodeKeys(db sildb.Database, prefix []byte) map[string]struct{} {
...
func (d peakBatchDB) NewBatch() sildb.Batch {
	return &peakBatch{Batch: d.Database.NewBatch(), peak: d.peak}
}

func (d peakBatchDB) NewBatchWithSize(size int) sildb.Batch {
...
func TestGenerateTrieBatchFlush(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if peak > 2*sildb.IdealBatchSize {
				t.Errorf("peak batch size %d exceeded 2*IdealBatchSize (%d); flush did not fire", peak, 2*sildb.IdealBatchSize)
			}
		})
	}
}
