...
func openRangeIterators(db sildb.Database, start common.Hash) *rangeIterators {
...
func (r *rangeIterators) flushIfFull(batch sildb.Batch, where string) error {
...
func openFlatIterator(db sildb.Database, prefix, start []byte, suffixLen int) *internal.HoldableIterator {
...
func reopenFlatIterator(db sildb.Database, old *internal.HoldableIterator, prefix []byte, suffixLen int) *internal.HoldableIterator {
	if !old.Next() {
		old.Release()
...
func generatePartition(ctx context.Context, cancel <-chan struct{}, db sildb.Database, scheme string, partition byte, rangeStart, rangeEnd common.Hash, c *genCounters) ([]byte, error) {
	iters := openRangeIterators(db, rangeStart)
	defer iters.release()

	batch := db.NewBatchWithSize(sildb.IdealBatchSize)
...
func GenerateTrie(db sildb.Database, scheme string, root common.Hash, cancel <-chan struct{}) (GenerateStats, error) {
...
func GenerateTrieWithProgress(db sildb.Database, scheme string, root common.Hash, cancel <-chan struct{}, prog *atomic.Uint64) (GenerateStats, error) {
	var (
		start        = time.Now()
		c            genCounters
...
func assembleRoot(db sildb.Database, scheme string, partitionBlobs [numPartitions][]byte) (common.Hash, error) {
	var (
		populated int
