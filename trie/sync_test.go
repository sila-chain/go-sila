...
func makeTestTrie(scheme string) (sildb.Database, *testDb, *StateTrie, map[string][]byte) {
...
// content map.
func checkTrieContents(t *testing.T, db sildb.Database, scheme string, root []byte, content map[string][]byte, rawTrie bool) {
...
func checkTrieConsistency(db sildb.Database, scheme string, root common.Hash, rawTrie bool) error {
	ndb := newTestDatabase(db, scheme)
	var it NodeIterator
	if rawTrie {
...
func syncWith(t *testing.T, root common.Hash, db sildb.Database, srcDb *testDb) {
	syncWithHookWriter(t, root, db, srcDb, nil)
}

func syncWithHookWriter(t *testing.T, root common.Hash, db sildb.Database, srcDb *testDb, hookWriter sildb.KeyValueWriter) {
	// Create a destination trie and sync with the scheduler
