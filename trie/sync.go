...
type Sync struct {
	...
	database sildb.KeyValueReader         // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database sildb.KeyValueReader, callback LeafCallback, scheme string) *Sync {
	ts := &Sync{
		scheme:   scheme,
		database: database,
...
// both sil/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw sildb.Batch) error {
