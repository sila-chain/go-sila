...
func (t *Trie) Prove(key []byte, proofDb sildb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *StateTrie) Prove(key []byte, proofDb sildb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb sildb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb sildb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, keys [][]byte, values [][]byte, proof sildb.KeyValueReader) (bool, error) {
