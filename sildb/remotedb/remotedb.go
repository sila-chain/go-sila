...
func (db *Database) ReadAncients(fn func(op sildb.AncientReaderOp) error) (err error) {
	return fn(db)
}
...
func (db *Database) ModifyAncients(f func(sildb.AncientWriteOp) error) (int64, error) {
	panic("not supported")
}
...
func New(client *rpc.Client) sildb.Database {
	if client == nil {
		return nil
	}
