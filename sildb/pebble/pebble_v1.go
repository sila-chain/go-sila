...
func NewV1(file string, cache int, handles int, namespace string, readonly bool) (*V1Database, error) {
	...
	db := &V1Database{
		...
		// of course). Sila is expected to handle recovery from an unclean shutdown.
	...
	opt := &pebble.Options{
		...
		WALBytesPerSync: 5 * sildb.IdealBatchSize,
	...
	// Disable seek compaction explicitly. Check https://github.com/sila-chain/go-sila/pull/20130
	// for more details.
...
func (d *V1Database) NewBatch() sildb.Batch {
	return &v1batch{
		b:  d.db.NewBatch(),
		db: d,
	}
...
func (d *V1Database) NewBatchWithSize(size int) sildb.Batch {
	return &v1batch{
...
func (d *V1Database) NewIterator(prefix []byte, start []byte) sildb.Iterator {
...
func (b *v1batch) Replay(w sildb.KeyValueWriter) error {
	reader := b.b.Reader()
	for {
		...
		if kind == pebble.InternalKeyKindSet {
			...
			if rangeDeleter, ok := w.(sildb.KeyValueRangeDeleter); ok {
				if err = rangeDeleter.DeleteRange(k, v); err != nil {
					return err
				}
			} else {
				return errors.New("sildb.KeyValueWriter does not implement DeleteRange")
			}
		} else {
