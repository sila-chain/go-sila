...
func (db *Database) NewBatch() sildb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewBatchWithSize(size int) sildb.Batch {
	return &batch{
		db: db.db,
...
func (db *Database) NewIterator(prefix []byte, start []byte) sildb.Iterator {
...
func (r *replayer) DeleteRange(start, end []byte) {
	...
	if rangeDeleter, ok := r.writer.(sildb.KeyValueRangeDeleter); ok {
		r.failure = rangeDeleter.DeleteRange(start, end)
	} else {
		r.failure = errors.New("sildb.KeyValueWriter does not implement DeleteRange")
	}
}
