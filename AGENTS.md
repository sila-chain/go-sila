...
Before committing, run the full test suite **without** `-short` to ensure all tests pass, including the Sila execution-spec tests and all state/block test permutations:
...
- `sil, rpc: make trace configs optional`
- `cmd/sila: add new flag for sync mode`
...
Examples:
- `core/vm: fix stack overflow in PUSH instruction`
- `core, sil: add arena allocator support`
- `cmd/sila, internal/silapi: refactor transaction args`
