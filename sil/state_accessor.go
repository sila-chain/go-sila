...
func (sil *Sila) hashState(ctx context.Context, block *types.Block, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	var (
		current  *types.Block
	...
	if base != nil {
		if preferDisk {
			...
			tdb := triedb.NewDatabase(sil.chainDb, triedb.HashDefaults)
		...
		current = sil.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		for i := uint64(0); i < reexecLimit; i++ {
			...
			parent := sil.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
...
//     on disk.
func (sil *Sila) stateAtBlock(ctx context.Context, block *types.Block, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	if sil.blockchain.TrieDB().Scheme() == rawdb.HashScheme {
		return sil.hashState(ctx, block, base, readOnly, preferDisk)
	}
	return sil.pathState(block)
}
...
func (sil *Sila) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int) (*types.Transaction, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := sil.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, release, err := sil.stateAtBlock(ctx, parent, nil, true, false)
	if err != nil {
	...
	signer := types.MakeSigner(sil.blockchain.Config(), block.Number(), block.Time())
