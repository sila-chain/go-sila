...
func TestSupplyGenesisAlloc(t *testing.T) {
	var (
		...
		sil1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Sila))
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			GenesisAlloc: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Sila))),
		},
		Number:     0,
...
func TestSupplyRewards(t *testing.T) {
	...
	expected := supplyInfo{
		Issuance: &supplyInfoIssuance{
			Reward: (*hexutil.Big)(new(big.Int).Mul(common.Big2, big.NewInt(params.Sila))),
		},
		Number:     1,
...
func TestSupplyRewardsWithUncle(t *testing.T) {
	...
	baseReward := silash.SilaConstantinopleBlockReward.ToBig()
...
func TestSupplyEip1559Burn(t *testing.T) {
	var (
		...
		// A sender who makes transactions, has some sil1
		key1, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		addr1   = crypto.PubkeyToAddress(key1.PublicKey)
	...
	var (
		...
		reward   = new(big.Int).Mul(common.Big2, big.NewInt(params.Sila))
...
// Contract A calls contract B which selfdestructs, but B receives sil1
// after the selfdestruct opcode executes from Contract A.
// Because Contract B is removed only at the end of the transaction
// the sila sent in between is burnt before SilaCancun hard fork.
func TestSupplySelfdestruct(t *testing.T) {
	...
	// 1. Test pre SilaCancun
	preSilaCancunOutput, preSilaCancunChain, err := testSupplyTracer(t, gspec, testBlockGenerationFunc, 1)
	if err != nil {
		t.Fatalf("Pre-cancun failed to test supply tracer: %v", err)
	}

	// Check balance at state:
	// 1. 0x0000...000dad has 1 sila
	// 2. A has 0 sila
	// 3. B has 0 sila
	statedb, _ := preSilaCancunChain.State()
	if got, exp := statedb.GetBalance(dad), sil1; got.CmpBig(exp) != 0 {
	...
	head := preSilaCancunChain.CurrentBlock()
	...
	actual := preSilaCancunOutput[expected.Number]

	compareAsJSON(t, expected, actual)

	// 2. Test post SilaCancun
	cancunTime := uint64(0)
	gspec.Config.SilaShanghaiTime = &cancunTime
	gspec.Config.SilaCancunTime = &cancunTime
	gspec.Config.BlobScheduleConfig = params.DefaultBlobSchedule

	postSilaCancunOutput, postSilaCancunChain, err := testSupplyTracer(t, gspec, testBlockGenerationFunc, 1)
	if err != nil {
	...
	// 3. A has 0 sila
	// 3. B has 5 gwei
	statedb, _ = postSilaCancunChain.State()
	if got, exp := statedb.GetBalance(dad), sil1; got.CmpBig(exp) != 0 {
	...
	// Check live trace output
	head = postSilaCancunChain.CurrentBlock()
	expected = supplyInfo{
		Burn: &supplyInfoBurn{
			SIP1559: (*hexutil.Big)(big.NewInt(55289500000000)),
		},
	...
	actual = postSilaCancunOutput[expected.Number]
...
//   - Contract B selfdestructs and sends the sil1 to itself (Burn amount to be counted).
//   - Contract C selfdestructs and sends the sil1 to itself.
...
func TestSupplySelfdestructItselfAndRevert(t *testing.T) {
	var (
		...
		gwei5   = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei))
		sil1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Sila))
		sil2    = new(big.Int).Mul(common.Big2, big.NewInt(params.Sila))
		sil5    = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.Sila))
	...
	// Check balance at state:
	// 1. A has 0 sila
	// 2. B has 0 sila, burned
	// 3. C has 2 sila, selfdestructed but parent D reverted
	// 4. D has 1 sila, reverted
	...
	if got, exp := statedb.GetBalance(cc), sil1; got.CmpBig(exp) != 0 {
		t.Fatalf("address \"%v\" balance, got %v exp %v\n", bb, got, exp)
	}
	if got, exp := statedb.GetBalance(dd), sil2; got.CmpBig(exp) != 0 {
		t.Fatalf("address \"%v\" balance, got %v exp %v\n", bb, got, exp)
	}
