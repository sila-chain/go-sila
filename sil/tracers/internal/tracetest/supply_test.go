...
// the ether sent in between is burnt before SilaCancun hard fork.
func TestSupplySelfdestruct(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Pre-sila_cancun failed to test supply tracer: %v", err)
	}
	...
	if got, exp := statedb.GetBalance(dad), eth1; got.CmpBig(exp) != 0 {
		t.Fatalf("Pre-sila_cancun address \"%v\" balance, got %v exp %v\n", dad, got, exp)
	}
	...
	if err != nil {
		t.Fatalf("Post-sila_cancun failed to test supply tracer: %v", err)
	}
	...
	if got, exp := statedb.GetBalance(dad), eth1; got.CmpBig(exp) != 0 {
		t.Fatalf("Post-sila_shanghai address \"%v\" balance, got %v exp %v\n", dad, got, exp)
	}
	if got, exp := statedb.GetBalance(aa), big.NewInt(0); got.CmpBig(exp) != 0 {
		t.Fatalf("Post-sila_shanghai address \"%v\" balance, got %v exp %v\n", aa, got, exp)
	}
...
func TestSupplySelfdestructItselfAndRevert(t *testing.T) {
	var (
		...
		gwei5   = new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei))
		eth1    = new(big.Int).Mul(common.Big1, big.NewInt(params.Ether))
		sil2    = new(big.Int).Mul(common.Big2, big.NewInt(params.Ether))
