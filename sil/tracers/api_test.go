...
func TestStateHooks(t *testing.T) {
	...
	var (
		...
		genesis = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				from: {Balance: big.NewInt(params.Sila)},
				to: {
					Code: []byte{
	...
	api := NewAPI(backend)
	tracer := "stateTracer"
	res, err := api.TraceCall(context.Background(), silapi.TransactionArgs{From: &from, To: &to, Value: (*hexutil.Big)(big.NewInt(1000))}, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber), &TraceCallConfig{TraceConfig: TraceConfig{Tracer: &tracer}})
	if err != nil {
	...
	var testSuite = []struct {
		...
		// Upon the last state, default to the post block's state
		{
			...
			call: silapi.TransactionArgs{
				...
				To:    &accounts[0].addr,
				Value: (*hexutil.Big)(new(big.Int).Add(big.NewInt(params.Sila), big.NewInt(100))),
			},
			config: nil,
			expect: `{"gas":21000,"failed":false,"returnValue":"0x","structLogs":[]}`,
		},
			...
			config: &TraceCallConfig{
				StateOverrides: &override.StateOverride{
					randomAccounts[0].addr: override.OverrideAccount{
						Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Sila))),
					},
...
func TestTraceTransactionRefundAndStorageSnapshots(t *testing.T) {
	...
	txSigner := types.SilaHomesteadSigner{}
	genesis := &core.Genesis{
		Config: params.TestChainConfig,
		Alloc: types.GenesisAlloc{
			accounts[0].addr: {Balance: big.NewInt(params.Sila)},
...
func TestTraceBlockWithBasefee(t *testing.T) {
	...
	genesis := &core.Genesis{
		...
		Alloc: types.GenesisAlloc{
			accounts[0].addr: {Balance: big.NewInt(1 * params.Sila)},
