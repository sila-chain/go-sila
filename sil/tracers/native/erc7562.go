...
func init() {
	tracers.DefaultDirectory.Register("src7562Tracer", newErc7562Tracer, false)
}
...
type src7562Tracer struct {
	config    src7562TracerConfig
	gasLimit  uint64
...
type src7562TracerConfig struct {
	...
	WithLog           bool             `json:"withLog"`        // If true, src7562 tracer will collect event logs
}

func getFullConfiguration(partial src7562TracerConfig) src7562TracerConfig {
	config := partial
...
func newErc7562TracerObject(cfg json.RawMessage) (*src7562Tracer, error) {
	var config src7562TracerConfig
	if cfg != nil {
...
func (t *src7562Tracer) OnTxStart(env *tracing.VMContext, tx *types.Transaction, from common.Address) {
	t.env = env
	t.gasLimit = tx.Gas()
}
...
func (t *src7562Tracer) captureEnd(output []byte, err error, reverted bool) {
...
func (t *src7562Tracer) OnTxEnd(receipt *types.Receipt, err error) {
	if t.interrupt.Load() {
		return
	}
...
func (t *src7562Tracer) OnLog(log1 *types.Log) {
...
func (t *src7562Tracer) GetResult() (json.RawMessage, error) {
	if t.interrupt.Load() {
...
func (t *src7562Tracer) Stop(err error) {
	t.reason.Store(&err)
...
func (t *src7562Tracer) clearFailedLogs(cf *callFrameWithOpcodes, parentFailed bool) {
...
func (t *src7562Tracer) OnOpcode(pc uint64, op byte, gas, cost uint64, scope tracing.OpContext, rData []byte, depth int, err error) {
	if t.interrupt.Load() {
		return
	}
	var (
...
func (t *src7562Tracer) handleReturnRevert(opcode vm.OpCode) {
	if opcode == vm.REVERT || opcode == vm.RETURN {
		t.lastOpWithStack = nil
	}
}

func (t *src7562Tracer) handleGasObserved(opcode vm.OpCode, currentCallFrame *callFrameWithOpcodes) {
	// [OP-012]
...
func (t *src7562Tracer) storeUsedOpcode(opcode vm.OpCode, currentCallFrame *callFrameWithOpcodes) {
...
func (t *src7562Tracer) handleStorageAccess(opcode vm.OpCode, scope tracing.OpContext, currentCallFrame *callFrameWithOpcodes) {
...
func (t *src7562Tracer) storeKeccak(opcode vm.OpCode, scope tracing.OpContext) {
	if opcode == vm.KECCAK256 {
		...
		if err != nil {
			log.Warn("src7562Tracer: failed to copy keccak preimage from memory", "err", err)
			return
		}
...
func (t *src7562Tracer) handleExtOpcodes(opcode vm.OpCode, currentCallFrame *callFrameWithOpcodes) {
...
func (t *src7562Tracer) handleAccessedContractSize(opcode vm.OpCode, scope tracing.OpContext, currentCallFrame *callFrameWithOpcodes) {
	// [OP-041]
...
func (t *src7562Tracer) isIgnoredOpcode(opcode vm.OpCode) bool {
