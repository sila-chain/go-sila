...
// NewAPI creates a new API definition for the tracing methods of the Sila service.
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if !found {
		...
		if !api.backend.TxIndexDone() {
			return nil, silapi.NewTxIndexingError()
		}
...
func overrideConfig(original *params.ChainConfig, override *params.ChainConfig) (*params.ChainConfig, bool) {
	...
	*copy = *original
	...
	// Apply forks (after SilaBerlin) to the copy.
	if block := override.SilaBerlinBlock; block != nil {
		copy.SilaBerlinBlock = block
		canon = false
	}
	if block := override.SilaLondonBlock; block != nil {
		copy.SilaLondonBlock = block
		canon = false
	}
	...
	if timestamp := override.SilaShanghaiTime; timestamp != nil {
		copy.SilaShanghaiTime = timestamp
		canon = false
	}
	if timestamp := override.SilaCancunTime; timestamp != nil {
		copy.SilaCancunTime = timestamp
		canon = false
	}
	if timestamp := override.SilaPragueTime; timestamp != nil {
		copy.SilaPragueTime = timestamp
		canon = false
	}
	if timestamp := override.SilaOsakaTime; timestamp != nil {
		copy.SilaOsakaTime = timestamp
		canon = false
	}
