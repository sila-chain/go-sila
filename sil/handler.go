...
// support all the operations needed by the Sila chain protocols.
...
type handlerConfig struct {
	...
	Sync           silconfig.SyncMode     // Whether to snap or full sync
...
// newHandler returns a handler for all Sila chain management protocol.
func newHandler(config *handlerConfig) (*handler, error) {
	...
	if h.downloader.ConfigSyncMode() == silconfig.SnapSync && (config.Chain.Snapshots() == nil && config.Chain.TrieDB().Scheme() == rawdb.HashScheme) {
...
func (h *handler) runEthPeer(peer *sil.Peer, handler sil.Handler) error {
	...
	// Execute the Sila handshake
	if err := peer.Handshake(h.networkID, h.chain, h.blockRange.currentRange()); err != nil {
		peer.Log().Debug("Sila handshake failed", "err", err)
		return err
	}
	reject := false // reserved peer slots
	if h.downloader.ConfigSyncMode() == silconfig.SnapSync {
	...
	peer.Log().Debug("Sila peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Sila peer registration failed", "err", err)
		return err
	}
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in sil syncer", "err", err)
		return err
	}
	if snap != nil {
	...
	for number, hash := range h.requiredBlocks {
		resCh := make(chan *sil.Response)

		req, err := peer.RequestHeadersByNumber(number, 1, 0, false, resCh)
		if err != nil {
			return err
		}
		go func(number uint64, hash common.Hash, req *sil.Request) {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*sil.BlockHeadersRequest))
				if len(headers) == 0 {
...
// runSnapExtension registers a `snap` peer into the joint sil/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `sil`, all subsystem registrations and lifecycle management will be done by
// the main `sil` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Warn("Sila peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `sil` peer if it exists
	logger.Debug("Removing Sila peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Sila peer removal failed", "err", err)
	}
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*silPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*silPeer][]common.Hash) // Set peer->hash to announce
	...
	for _, tx := range txs {
		var directSet map[*silPeer]struct{}
		switch {
...
type blockRangeState struct {
	prev    sil.BlockRangeUpdatePacket
	next    atomic.Pointer[sil.BlockRangeUpdatePacket]
...
func (h *handler) blockRangeLoop(st *blockRangeState) {
	...
	for {
		select {
		case ev := <-st.syncCh:
			if ev.Type == downloader.SyncStarted && ev.Mode == silconfig.SnapSync {
...
func (st *blockRangeState) update(chain *core.BlockChain, latest *types.Header) {
	...
	st.next.Store(&sil.BlockRangeUpdatePacket{
...
func (st *blockRangeState) currentRange() sil.BlockRangeUpdatePacket {
	return *st.next.Load()
}
...
func newBroadcastChoice(self enode.ID, key [16]byte) *broadcastChoice {
	return &broadcastChoice{
		self:   self,
		key:    key,
		buffer: make(map[*silPeer]struct{}),
	}
}
...
func (bc *broadcastChoice) choosePeers(peers []*silPeer, txSender common.Address) map[*silPeer]struct{} {
