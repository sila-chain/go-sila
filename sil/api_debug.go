...
// DebugAPI is the collection of Sila full node APIs for debugging the
// protocol.
...
func NewDebugAPI(sil *Sila) *DebugAPI {
	return &DebugAPI{sil: sil}
}
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, _, stateDb := api.sil.miner.Pending()
		if stateDb == nil {
	...
	case rpc.LatestBlockNumber:
		header = api.sil.blockchain.CurrentBlock()
	case rpc.FinalizedBlockNumber:
		header = api.sil.blockchain.CurrentFinalBlock()
	case rpc.SafeBlockNumber:
		header = api.sil.blockchain.CurrentSafeBlock()
	default:
		block := api.sil.blockchain.GetBlockByNumber(uint64(blockNr))
		if block == nil {
...
func (api *DebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.sil.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		blocks  = rawdb.ReadAllBadBlocks(api.sil.chainDb)
	...
	for _, block := range blocks {
		...
		blockJSON = silapi.RPCMarshalBlock(block, true, true, api.sil.APIBackend.ChainConfig())
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			case rpc.SafeBlockNumber:
				header = api.sil.blockchain.CurrentSafeBlock()
			default:
				block := api.sil.blockchain.GetBlockByNumber(uint64(number))
				if block == nil {
...
func (api *DebugAPI) StorageRangeAt(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	...
	block, err := api.sil.APIBackend.BlockByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	_, _, statedb, release, err := api.sil.stateAtTransaction(ctx, block, txIndex)
	if err != nil {
		return StorageRangeResult{}, err
	}
	defer release()
...
func (api *DebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startHeader = api.sil.blockchain.GetHeaderByNumber(startNum)
	if startHeader == nil {
	...
	if endNum == nil {
		...
		startHeader = api.sil.blockchain.GetHeaderByHash(startHeader.ParentHash)
		if startHeader == nil {
			return nil, fmt.Errorf("block %x has no parent", endHeader.Number)
		}
	} else {
		endHeader = api.sil.blockchain.GetHeaderByNumber(*endNum)
		if endHeader == nil {
...
func (api *DebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startHeader, endHeader *types.Header
	startHeader = api.sil.blockchain.GetHeaderByHash(startHash)
	if startHeader == nil {
	...
	if endHash == nil {
		...
		endHeader = api.sil.blockchain.GetHeaderByHash(*endHash)
		if endHeader == nil {
...
func (api *DebugAPI) getModifiedAccounts(startHeader, endHeader *types.Header) ([]common.Address, error) {
	...
	triedb := api.sil.BlockChain().TrieDB()
...
func (api *DebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	if api.sil.blockchain.TrieDB().Scheme() == rawdb.PathScheme {
	...
	var resolveNum = func(num rpc.BlockNumber) (uint64, error) {
		...
		if num.Int64() < 0 {
			block := api.sil.blockchain.CurrentBlock()
			if block == nil {
	...
	for i := int64(start); i != int64(end); i += delta {
		...
		h := api.sil.BlockChain().GetHeaderByNumber(uint64(i))
		if h == nil {
			return 0, fmt.Errorf("missing header %d", i)
		}
		if ok, _ := api.sil.ChainDb().Has(h.Root[:]); ok {
			return uint64(i), nil
		}
	}
...
func (api *DebugAPI) SetTrieFlushInterval(interval string) error {
	...
	api.sil.blockchain.SetTrieFlushInterval(t)
	return nil
}
...
func (api *DebugAPI) GetTrieFlushInterval() (string, error) {
	...
	return api.sil.blockchain.GetTrieFlushInterval().String(), nil
}
...
func (api *DebugAPI) StateSize(blockHashOrNumber *rpc.BlockNumberOrHash) (interface{}, error) {
	sizer := api.sil.blockchain.StateSizer()
	if sizer == nil {
	...
	if blockHashOrNumber == nil {
		...
		header, herr := api.sil.APIBackend.HeaderByNumberOrHash(context.Background(), *blockHashOrNumber)
