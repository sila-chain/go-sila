...
func (h *testEthHandler) Handle(peer *sil.Peer, packet sil.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected sil packet type in tests: %T", packet))
	}
}
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	silNoFork.Start(1000)
	silProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer silNoFork.Stop()
	defer silProFork.Stop()
	...
	// Progress into SilaHomestead. Fork's match, so we don't care what the future holds
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	handler := newTestHandler(silconfig.FullSync)
	defer handler.close()
	...
	if err := src.Handshake(1, handler.chain, sil.BlockRangeUpdatePacket{EarliestBlock: 0, LatestBlock: head.Number.Uint64(), LatestBlockHash: head.Hash()}); err != nil {
		t.Fatalf("failed to run protocol handshake")
	}
	// Send the transaction to the sink and verify that it's added to the tx pool
	tx := types.NewTransaction(0, common.Address{}, big.NewInt(0), 100000, big.NewInt(0), nil)
	tx, _ = types.SignTx(tx, types.SilaHomesteadSigner{}, testKey)
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	// Run the handshake locally to avoid spinning up a source handler
	head := handler.chain.CurrentBlock()
	if err := sink.Handshake(1, handler.chain, sil.BlockRangeUpdatePacket{EarliestBlock: 0, LatestBlock: head.Number.Uint64(), LatestBlockHash: head.Hash()}); err != nil {
		t.Fatalf("failed to run protocol handshake")
	}
	...
	for len(seen) < len(insert) {
		...
		case <-bcasts:
			t.Errorf("initial tx broadcast received on post sil/66")
		}
	}
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	source := newTestHandler(silconfig.FullSync)
	defer source.close()

	sinks := make([]*testHandler, 10)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = newTestHandler(silconfig.FullSync)
		defer sinks[i].close()
