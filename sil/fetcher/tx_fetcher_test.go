...
// matching each announcer's protocol version: full below sil/72, without
// blobs on sil/72.
func TestTransactionFetcherBlobSizeVersions(t *testing.T) {
	...
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			doTxNotify{peer: "A", version: sil.SIL72, hashes: []common.Hash{tx.Hash()}, types: []byte{types.BlobTxType}, sizes: []uint32{sizeWithoutBlob}},
			doTxNotify{peer: "B", version: sil.SIL71, hashes: []common.Hash{tx.Hash()}, types: []byte{types.BlobTxType}, sizes: []uint32{size}},
			doTxNotify{peer: "C", version: sil.SIL72, hashes: []common.Hash{tx.Hash()}, types: []byte{types.BlobTxType}, sizes: []uint32{size}},
			doWait{time: 0, step: true}, // zero time, but the blob fetching should be scheduled

			// Only C, announcing the wrong form for its version, may be dropped.
			doTxEnqueue{peer: "B", version: sil.SIL71, txs: []*types.Transaction{tx}, direct: true},
			doFunc(func() {
				if peer := <-drop; peer != "C" {
...
func TestTransactionForgotten(t *testing.T) {
	...
	if err := fetcher.Enqueue("peer", sil.SIL70, []*types.Transaction{tx1, tx2}, false); err != nil {
		t.Fatal(err)
	}
	...
	if err := fetcher.Enqueue("peer", sil.SIL70, []*types.Transaction{tx1}, false); err != nil {
		t.Fatal(err)
	}
	if !fetcher.isKnownUnderpriced(tx1.Hash()) {
