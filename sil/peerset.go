...
// the `sil` protocol, with or without the `snap` extension.
type peerSet struct {
	peers map[string]*silPeer // Peers connected on the `sil` protocol

	snapWait map[string]chan *snap.Peer // Peers connected on `sil` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `sil`
...
// registerSnapExtension unblocks an already connected `sil` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `sil` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	...
	// Inject the peer into an `sil` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *sil.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `sil` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *sil.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *silPeer {
	ps.lock.RLock()
...
// len returns if the current number of `sil` peers in the set. Since the `snap`
// peers are tied to the existence of an `sil` connection, that will always be a
// subset of `sil`.
