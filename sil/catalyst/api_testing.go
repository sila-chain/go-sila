...
func newTestingAPI(backend *sil.Sila) rpc.API {
	return rpc.API{
		Namespace:     "testing",
...
func (api *testingAPI) BuildBlockV1(parentHash common.Hash, payloadAttributes engine.PayloadAttributes, transactions *[]hexutil.Bytes, extraData *hexutil.Bytes) (*engine.ExecutionPayloadEnvelope, error) {
	if api.sil.BlockChain().CurrentBlock().Hash() != parentHash {
...
func (api *testingAPI) CommitBlockV1(ctx context.Context, payloadAttributes engine.PayloadAttributes, transactions *[]hexutil.Bytes, extraData *hexutil.Bytes) (common.Hash, error) {
	...
	if _, err := api.sil.BlockChain().InsertBlockWithoutSetHead(ctx, block, false); err != nil {
		return common.Hash{}, err
	}
	if _, err := api.sil.BlockChain().SetCanonical(block); err != nil {
		return common.Hash{}, err
	}
...
func (api *testingAPI) buildTestingBlock(payloadAttributes engine.PayloadAttributes, transactions *[]hexutil.Bytes, extraData *hexutil.Bytes) (*types.Block, *engine.ExecutionPayloadEnvelope, error) {
	parentHash := api.sil.BlockChain().CurrentBlock().Hash()
	...
	return api.sil.Miner().BuildTestingPayload(args, txs, buildEmpty, extra)
}
