...
func TestSimulatedBeaconSendWithdrawals(t *testing.T) {
	...
	subscription := silService.BlockChain().SubscribeChainHeadEvent(chainHeadCh)
	...
	for i := 0; i < 20; i++ {
		...
		if err := silService.APIBackend.SendTx(context.Background(), tx); err != nil {
	...
	for {
		select {
		case ev := <-chainHeadCh:
			block := silService.BlockChain().GetBlock(ev.Header.Hash(), ev.Header.Number.Uint64())
...
func TestOnDemandSpam(t *testing.T) {
	...
	var (
		...
		sub                    = sil.BlockChain().SubscribeChainHeadEvent(chainHeadCh)
	)
	defer node.Close()
	...
	go func() {
		for i := 0; i < txCount; i++ {
			...
			if err := sil.TxPool().Add([]*types.Transaction{tx}, false)[0]; err != nil {
	...
	for {
		select {
		case ev := <-chainHeadCh:
			block := sil.BlockChain().GetBlock(ev.Header.Hash(), ev.Header.Number.Uint64())
