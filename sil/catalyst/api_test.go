...
func startEthService(t testing.TB, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *sil.Sila) {
	...
	ethcfg := &silconfig.Config{
		Genesis:        genesis,
		SyncMode:       silconfig.FullSync,
...
func assembleEnvelope(api *ConsensusAPI, parentHash common.Hash, params *engine.PayloadAttributes) (*engine.ExecutionPayloadEnvelope, error) {
	...
	payload, err := api.sil.Miner().BuildPayload(context.Background(), args, false)
	if err != nil {
		return nil, err
	}
...
// TestWithdrawals creates and verifies two post-SilaShanghai blocks. The first
...
func setupBodies(t *testing.T) (*node.Node, *sil.Sila, []*types.Block) {
	genesis, blocks := generateMergeChain(10, true)
...
func TestGetBlockBodiesByHash(t *testing.T) {
	...
	tests := []struct {
		...
		// First pow block
		{
			results: []*types.Body{sil.BlockChain().GetBlockByNumber(0).Body()},
			hashes:  []common.Hash{sil.BlockChain().GetBlockByNumber(0).Hash()},
		},
		// Last pow block
		{
...
func newGetBlobEnv(t testing.TB, version byte) (*node.Node, *ConsensusAPI) {
	...
	// Disable SilaOsaka fork for GetBlobsV1
	if version == 0 {
