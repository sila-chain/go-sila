...
// version: 0 = BlobSidecarVersion0 (pre-SilaOsaka), 1 = BlobSidecarVersion1 (SilaOsaka+)
...
func newBenchmarkBlobEnv(b *testing.B, blobCount int, version byte, fork benchFork) *benchmarkBlobEnv {
	...
	config.BlobScheduleConfig = &params.BlobScheduleConfig{
		SilaCancun: &params.BlobConfig{Target: 6, Max: 128, UpdateFraction: 3338477},
		SilaPrague: &params.BlobConfig{Target: 6, Max: 128, UpdateFraction: 5007716},
	}
	// Configure fork times based on requested fork
	switch fork {
	case forkSilaCancun:
		config.SilaPragueTime = nil
...
func (env *benchmarkBlobEnv) addBlobTxs(b *testing.B) {
	...
	errs := env.sil.TxPool().Add(txs, true)
...
// GetBlobsV1 is available at SilaCancun/SilaPrague (pre-SilaOsaka).
	...
	for _, blobCount := range benchmarkBlobCounts {
		for _, enc := range encodingTypes {
			b.Run(fmt.Sprintf("blobs=%d/enc=%s", blobCount, enc), func(b *testing.B) {
				env := newBenchmarkBlobEnv(b, blobCount, 0, forkSilaPrague)
				defer env.Close()
...
// GetBlobsV2 is available at SilaOsaka+.
func BenchmarkGetBlobsV2Extended(b *testing.B) {
	for _, blobCount := range benchmarkBlobCounts {
		for _, enc := range encodingTypes {
			b.Run(fmt.Sprintf("blobs=%d/enc=%s", blobCount, enc), func(b *testing.B) {
				env := newBenchmarkBlobEnv(b, blobCount, 1, forkSilaOsaka)
				defer env.Close()
...
// GetBlobsV3 is available at SilaOsaka+.
...
// BenchmarkGetPayloadV5WithBlobs benchmarks GetPayloadV5 (SilaOsaka fork) with blobs.
// Note: Measures single iteration performance due to NewPayload complexity at SilaOsaka.
	...
	for _, blobCount := range benchmarkBlobCounts {
		for _, enc := range encodingTypes {
			b.Run(fmt.Sprintf("blobs=%d/enc=%s", blobCount, enc), func(b *testing.B) {
				...
				parent := env.api.sil.BlockChain().CurrentHeader()
...
func BenchmarkNewPayloadV3WithBlobs(b *testing.B) {
	for _, blobCount := range benchmarkBlobCounts {
		for _, enc := range encodingTypes {
			b.Run(fmt.Sprintf("blobs=%d/enc=%s", blobCount, enc), func(b *testing.B) {
				env := newBenchmarkBlobEnv(b, blobCount, 0, forkSilaCancun)
				defer env.Close()
...
// BenchmarkFullBlobWorkflowSilaOsaka benchmarks the complete blob workflow at SilaOsaka:
// ForkchoiceUpdated -> GetPayload
// Note: Measures single iteration performance due to NewPayload complexity at SilaOsaka.
func BenchmarkFullBlobWorkflowSilaOsaka(b *testing.B) {
	for _, blobCount := range benchmarkBlobCounts {
		for _, enc := range encodingTypes {
			b.Run(fmt.Sprintf("blobs=%d/enc=%s", blobCount, enc), func(b *testing.B) {
				env := newBenchmarkBlobEnv(b, blobCount, 1, forkSilaOsaka)
