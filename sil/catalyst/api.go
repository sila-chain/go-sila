...
func Register(stack *node.Node, backend *sil.Sila) error {
...
func NewConsensusAPI(sil *sil.Sila) *ConsensusAPI {
	api := newConsensusAPIWithoutHeartbeat(sil)
	go api.heartbeat()
	return api
}

// newConsensusAPIWithoutHeartbeat creates a new consensus api for the SimulatedBeacon Node.
func newConsensusAPIWithoutHeartbeat(sil *sil.Sila) *ConsensusAPI {
	if sil.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	sil.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	return api
}
...
func (api *ConsensusAPI) forkchoiceUpdated(ctx context.Context, update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes, payloadVersion engine.PayloadVersion, payloadWitness bool) (result engine.ForkChoiceResponse, err error) {
	...
	// reason.
	block := api.sil.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		if header == nil {
			// The head hash is unknown locally, try to resolve it from the `sil` network
			log.Warn("Fetching the unknown forkchoice head from network", "hash", update.HeadBlockHash)
			retrievedHead, err := api.sil.Downloader().GetHeader(update.HeadBlockHash)
			if err != nil {
		...
		if finalized == nil {
			finalized = api.sil.BlockChain().GetHeaderByHash(update.FinalizedBlockHash)
		}
		...
		if err := api.sil.Downloader().BeaconSync(header, finalized); err != nil {
	...
	if block.Difficulty().BitLen() > 0 && block.NumberU64() > 0 {
		ph := api.sil.BlockChain().GetHeader(block.ParentHash(), block.NumberU64()-1)
		if ph == nil {
	...
	if rawdb.ReadCanonicalHash(api.sil.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.sil.BlockChain().SetCanonical(block); err != nil {
			return engine.ForkChoiceResponse{PayloadStatus: engine.PayloadStatusV1{Status: engine.INVALID, LatestValidHash: &latestValid}}, err
		}
	} else if api.sil.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		...
		finalBlock := api.sil.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
			log.Warn("Final block not available in database", "hash", update.FinalizedBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not available in database"))
		} else if rawdb.ReadCanonicalHash(api.sil.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
		...
		api.sil.BlockChain().SetFinalized(finalBlock.Header())
	}
	// Check if the safe block hash is in our canonical tree, if not something is wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.sil.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.sil.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.sil.BlockChain().SetSafe(safeBlock.Header())
	}
	...
	if payloadAttributes != nil {
		...
		payload, err := api.sil.Miner().BuildPayload(ctx, args, payloadWitness)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.sil.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) GetBlobsV1(ctx context.Context, hashes []common.Hash) (result engine.BlobAndProofListV1, err error) {
	...
	blobs, _, proofs, err := api.sil.BlobCache().GetBlobs(ctx, hashes, types.BlobSidecarVersion0)
	if err != nil {
...
func (api *ConsensusAPI) getBlobs(ctx context.Context, hashes []common.Hash, v2 bool) (result engine.BlobAndProofListV2, err error) {
	...
	available := 0
	for _, ok := range api.sil.BlobCache().HasBlobs(ctx, hashes) {
		if ok {
			available++
		}
	}
	...
	// heavy disk I/O.
	blobs, _, proofs, err := api.sil.BlobCache().GetBlobs(ctx, hashes, types.BlobSidecarVersion1)
	if err != nil {
		return nil, engine.InvalidParams.With(err)
	}
...
func (api *ConsensusAPI) newPayload(ctx context.Context, params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, requests [][]byte, witness bool) (result engine.PayloadStatusV1, err error) {
	...
	//      api.sil.BlockChain().InsertBlockWithoutSetHead, where it is blocked on
	...
	if block := api.sil.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.sil.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	if api.sil.Downloader().ConfigSyncMode() == silconfig.SnapSync {
	...
	if !api.sil.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
	...
	start := time.Now()
	proofs, err := api.sil.BlockChain().InsertBlockWithoutSetHead(ctx, block, witness)
	...
	// Emit NewPayloadEvent for silstats reporting
	api.sil.BlockChain().SendNewPayloadEvent(core.NewPayloadEvent{
		Hash:           hash,
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) engine.PayloadStatusV1 {
	...
	err := api.sil.Downloader().BeaconExtend(block.Header())
	if err == nil {
	...
	if api.sil.Downloader().ConfigSyncMode() == silconfig.FullSync {
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *engine.PayloadStatusV1 {
	...
	if header := api.sil.BlockChain().GetHeader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) config() *params.ChainConfig {
	return api.sil.BlockChain().Config()
}
...
func getBodiesByRange[T any](api *ConsensusAPI, start, count hexutil.Uint64, getBody func(*types.Block) *T) ([]*T, error) {
	...
	current := api.sil.BlockChain().CurrentBlock().Number.Uint64()
