...
// SimulatedBeacon drives an Sila instance as if it were a real beacon
...
func NewSimulatedBeacon(period uint64, feeRecipient common.Address, sil *sil.Sila) (*SimulatedBeacon, error) {
	block := sil.BlockChain().CurrentBlock()
	current := engine.ForkchoiceStateV1{
		HeadBlockHash:      block.Hash(),
		SafeBlockHash:      block.Hash(),
		FinalizedBlockHash: block.Hash(),
	}
	engineAPI := newConsensusAPIWithoutHeartbeat(sil)

	// if genesis block, send forkchoiceUpdated to trigger transition to PoS
	if block.Number.Sign() == 0 {
		version := payloadVersion(sil.BlockChain().Config(), block.Time)
...
func (c *SimulatedBeacon) sealBlock(withdrawals []*types.Withdrawal, timestamp uint64) error {
	...
	if err := c.sil.APIBackend.TxPool().Sync(); err != nil {
		return fmt.Errorf("failed to sync txpool: %w", err)
	}

	version := payloadVersion(c.sil.BlockChain().Config(), timestamp)
	tracer := otel.Tracer("")
...
func (c *SimulatedBeacon) finalizedBlockHash(number uint64) *common.Hash {
	...
	if finalizedBlock := c.sil.BlockChain().GetBlockByNumber(finalizedNumber); finalizedBlock != nil {
...
func (c *SimulatedBeacon) Commit() common.Hash {
	...
	return c.sil.BlockChain().CurrentBlock().Hash()
}
...
func (c *SimulatedBeacon) Fork(parentHash common.Hash) error {
	...
	c.sil.TxPool().Sync()
	if pending, _ := c.sil.TxPool().Pending(txpool.PendingFilter{}); len(pending) != 0 {
		return errors.New("pending block dirty")
	}

	parent := c.sil.BlockChain().GetBlockByHash(parentHash)
	if parent == nil {
		return errors.New("parent not found")
	}
	_, err := c.sil.BlockChain().SetCanonical(parent)
	return err
}
...
func (c *SimulatedBeacon) AdjustTime(adjustment time.Duration) error {
	...
	parent := c.sil.BlockChain().CurrentBlock()
	if parent == nil {
		return errors.New("parent not found")
	}
	withdrawals := c.withdrawals.pop(10)
