...
func (b *testBackend) ChainDb() sildb.Database {
	return b.db
}
...
func newTestFilterSystem(db sildb.Database, cfg Config) (*testBackend, *FilterSystem) {
...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		_, chain, _ = core.GenerateChainWithGenesis(genesis, silash.NewFaker(), 10, func(i int, gen *core.BlockGen) {})
...
func TestPendingTxFilterFullTx(t *testing.T) {
	...
	for {
		...
		tx := results.([]*silapi.RPCTransaction)
		txs = append(txs, tx...)
...
func TestInvalidGetLogsRequest(t *testing.T) {
	...
	var (
		...
		db, blocks, _    = core.GenerateChainWithGenesis(genesis, silash.NewFaker(), 10, func(i int, gen *core.BlockGen) {})
	...
	blockchain, err := core.NewBlockChain(db, genesis, silash.NewFaker(), nil)
	if err != nil {
...
func TestExceedLogQueryLimit(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, gspec.ToBlock(), silash.NewFaker(), db, 1000, func(i int, gen *core.BlockGen) {})

	options := core.DefaultConfig().WithStateScheme(rawdb.HashScheme)
	options.TxLookupLimit = 0 // index all txs
	bc, err := core.NewBlockChain(db, gspec, silash.NewFaker(), options)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransactionReceiptsSubscription(t *testing.T) {
	...
	var (
		...
		signer       = types.NewSilaLondonSigner(big.NewInt(1))
		genesis      = &core.Genesis{
			Alloc:   types.GenesisAlloc{addr1: {Balance: big.NewInt(1000000000000000000)}}, // 1 SIL
			Config:  params.TestChainConfig,
			BaseFee: big.NewInt(params.InitialBaseFee),
		}
		_, chain, _ = core.GenerateChainWithGenesis(genesis, silash.NewFaker(), 1, func(i int, gen *core.BlockGen) {
