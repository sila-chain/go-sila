...
func benchmarkFilters(b *testing.B, history uint64, noHistory bool) {
	...
	defer db.Close()
	_, chain, receipts := core.GenerateChainWithGenesis(gspec, silash.NewFaker(), 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func testFilters(t *testing.T, history uint64, noHistory bool) {
	...
	pchain, preceipts := core.GenerateChain(gspec.Config, chain[len(chain)-1], silash.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {
...
func TestRangeLimit(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, gspec.ToBlock(), silash.NewFaker(), db, 10, func(i int, gen *core.BlockGen) {})
	options := core.DefaultConfig().WithStateScheme(rawdb.HashScheme)
