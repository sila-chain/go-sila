...
// information related to the Sila protocol such as blocks, transactions and logs.
...
// `sil_getFilterChanges` polling method that is also used for log filters.
			...
			case txs := <-txs:
				...
				for _, tx := range txs {
					if fullTx != nil && *fullTx {
						rpcTx := silapi.NewRPCPendingTransaction(tx, latest, chainConfig)
...
// It is part of the filter package since polling goes with sil_getFilterChanges.
...
func (api *FilterAPI) GetFilterChanges(id rpc.ID) (interface{}, error) {
	...
	if f, found := api.filters[id]; found {
		...
		case PendingTransactionsSubscription:
			if f.fullTx {
				txs := make([]*silapi.RPCTransaction, 0, len(f.txs))
				for _, tx := range f.txs {
					txs = append(txs, silapi.NewRPCPendingTransaction(tx, latest, chainConfig))
				}
				f.txs = nil
				return txs, nil
			} else {
