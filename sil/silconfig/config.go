...
// Package silconfig contains the configuration of the SIL and LES protocols.
package silconfig
...
// Config contains configuration options for SIL and LES protocols.
type Config struct {
	// The genesis block, which is inserted if the database is empty.
	// If nil, the Sila main net block is used.
	...
	// SIP-7966: sil_sendRawTransactionSync timeouts
...
// Clique is allowed for now to live standalone, but silash is forbidden and can
// only exist on already merged networks.
func CreateConsensusEngine(config *params.ChainConfig, db sildb.Database) (consensus.Engine, error) {
	...
	return beacon.New(silash.NewFaker()), nil
}
