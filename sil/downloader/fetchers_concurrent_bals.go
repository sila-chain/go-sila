...
// concurrent fetcher and the downloader. It fetches SIP-7928 block access
// lists on a best effort basis: only sil/71+ peers can serve them and blocks
...
// lists from the download queue to the specified peer. Peers below sil/71
...
func (q *balQueue) request(peer *peerConnection, req *fetchRequest, resCh chan *sil.Response) (*sil.Request, error) {
...
// downloader's queue.
func (q *balQueue) deliver(peer *peerConnection, packet *sil.Response) (int, error) {
	bals := *packet.Res.(*sil.BlockAccessListResponse)
