...
func makeChain(n int, seed byte, parent *types.Block, empty bool) ([]*types.Block, []types.Receipts) {
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, silash.NewFaker(), testDB, n, func(i int, block *core.BlockGen) {
		...
		for {
			...
			if f != nil {
				...
				for _, hdr := range f.Headers[0 : len(f.Headers)-numToSkip] {
					...
					bodies = append(bodies, sil.BlockBody{Transactions: txsList})
				}
				hashes := sil.BlockBodyHashes{
