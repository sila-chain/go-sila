...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *sil.Response) (*sil.Request, error)
	...
	deliver(peer *peerConnection, packet *sil.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue) error {
	...
	responses := make(chan *sil.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*sil.Request)
	defer func() {
	...
	ordering := make(map[*sil.Request]int)
	timeouts := prque.New[int64, *sil.Request](func(data *sil.Request, index int) {
		ordering[data] = index
	})
	...
	stales := make(map[string]*sil.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
