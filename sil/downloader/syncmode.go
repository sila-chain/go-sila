...
func newSyncModer(mode silconfig.SyncMode, chain BlockChain, disk sildb.KeyValueReader) *syncModer {
	if mode == silconfig.FullSync {
...
func (m *syncModer) get(report bool) silconfig.SyncMode {
	m.lock.Lock()
	defer m.lock.Unlock()

	// If we're in snap sync mode, return that directly
	if m.mode == silconfig.SnapSync {
		return silconfig.SnapSync
	}
	logger := log.Debug
	if report {
		logger = log.Info
...
func (m *syncModer) disableSnap() bool {
	...
	if m.mode == silconfig.FullSync {
		return false
	}
	m.mode = silconfig.FullSync
	return true
}
