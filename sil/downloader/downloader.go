...
// Deprecated: use silconfig.SyncMode instead
type SyncMode = silconfig.SyncMode

const (
	// Deprecated: use silconfig.FullSync
	FullSync = silconfig.FullSync
	// Deprecated: use silconfig.SnapSync
	SnapSync = silconfig.SnapSync
)
...
type Downloader struct {
	...
	stateDB sildb.Database // Database to state sync into (and deduplicate via)
...
func New(stateDb sildb.Database, mode silconfig.SyncMode, chain BlockChain, dropPeer peerDropFn, success func(), snapV2 bool) *Downloader {
...
func (d *Downloader) syncToHead() (err error) {
	...
	if mode == silconfig.SnapSync && pivot == nil {
	...
	if mode == silconfig.SnapSync && pivot.Number.Uint64() != 0 {
	...
	if mode == silconfig.SnapSync && d.chainCutoffNumber != 0 {
