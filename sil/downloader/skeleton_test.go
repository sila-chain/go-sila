...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *sil.Response) (*sil.Request, error) {
	...
	if headers == nil {
		...
		if len(p.headers) > int(origin) { // Don't serve headers if we're missing the origin
			for i := 0; i < amount; i++ {
				...
				// attack by sending/receiving those over sil.
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *sil.Response) (*sil.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *sil.Response) (*sil.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, []uint64, []uint64, chan *sil.Response) (*sil.Request, error) {
	panic("skeleton sync must not request receipts")
}

func (p *skeletonTestPeer) RequestBALs([]common.Hash, chan *sil.Response) (*sil.Request, error) {
...
func checkSkeletonProgress(db sildb.KeyValueReader, unpredictable bool, peers []*skeletonTestPeer, expected skeletonExpect) error {
	var progress skeletonProgress
