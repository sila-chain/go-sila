...
func newTester(t *testing.T, mode silconfig.SyncMode) *downloadTester {
	return newTesterWithNotification(t, mode, nil)
}

// newTesterWithNotification creates a new downloader test mocker (snap/1).
func newTesterWithNotification(t *testing.T, mode silconfig.SyncMode, success func()) *downloadTester {
...
func newTesterWithSnap(t *testing.T, mode silconfig.SyncMode, success func(), snapV2 bool) *downloadTester {
	gspec := &core.Genesis{
		Config:  params.TestChainConfig,
		Alloc:   types.GenesisAlloc{testAddress: {Balance: big.NewInt(1000000000000000)}},
		BaseFee: big.NewInt(params.InitialBaseFee),
	}
	return newTesterWithGenesis(t, mode, success, snapV2, gspec, silash.NewFaker())
}
...
func newTesterWithGenesis(t *testing.T, mode silconfig.SyncMode, success func(), snapV2 bool, gspec *core.Genesis, engine consensus.Engine) *downloadTester {
...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin common.Hash, amount int, skip int, reverse bool, sink chan *sil.Response) (*sil.Request, error) {
	// Service the header query via the live handler code
	rlpHeaders := sil.ServiceGetBlockHeadersQuery(dlp.chain, &sil.GetBlockHeadersRequest{
	...
	res := &sil.Response{
		Req:  req,
		Res:  (*sil.BlockHeadersRequest)(&headers),
		Meta: hashes,
		Time: 1,
...
// origin; associated with a particular peer in the download tester. The returned
// function can be used to retrieve batches of headers from the particular peer.
func (dlp *downloadTesterPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *sil.Response) (*sil.Request, error) {
	// Service the header query via the live handler code
	rlpHeaders := sil.ServiceGetBlockHeadersQuery(dlp.chain, &sil.GetBlockHeadersRequest{
		Origin: sil.HashOrNumber{
			Number: origin,
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash, sink chan *sil.Response) (*sil.Request, error) {
	blobs := sil.ServiceGetBlockBodiesQuery(dlp.chain, hashes)

	bodies := make([]*types.Body, len(blobs))
	ethbodies := make([]sil.BlockBody, len(blobs))
	...
	res := &sil.Response{
		Req: req,
		Res: (*sil.BlockBodiesResponse)(&ethbodies),
		Meta: sil.BlockBodyHashes{
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []common.Hash, gasUsed []uint64, timestamps []uint64, sink chan *sil.Response) (*sil.Request, error) {
	blobs := sil.ServiceGetReceiptsQuery69(dlp.chain, hashes)
	...
	resp := sil.ReceiptsRLPResponse(types.EncodeBlockReceiptLists(receipts))
	res := &sil.Response{
		Req:  &sil.Request{Peer: dlp.id},
		Res:  &resp,
		Meta: hashes,
		Time: 1,
		Done: make(chan error, 1), // Ignore the returned status
...
func (dlp *downloadTesterPeer) RequestBALs(hashes []common.Hash, sink chan *sil.Response) (*sil.Request, error) {
	var (
	...
	// deliver the response right away
	resp := sil.BlockAccessListResponse(bals)
	res := &sil.Response{
		Req:  &sil.Request{Peer: dlp.id},
		Res:  &resp,
		Meta: meta,
...
func makeBALChain(n int) (*core.Genesis, []*types.Block) {
	...
	gspec := &core.Genesis{
		...
		Alloc: types.GenesisAlloc{
			testAddress:                      {Balance: new(big.Int).Mul(big.NewInt(1000), big.NewInt(params.Sila))},
	...
	_, blocks, _ := core.GenerateChainWithGenesis(gspec, beacon.New(silash.NewFaker()), n, func(i int, block *core.BlockGen) {
		// The chain maker only executes the SIP-4788 system call when the
...
// Tests that block access lists are retrieved from sil/71+ peers during sync
...
// blocks without them.
func TestBALSynchronisationFull(t *testing.T)       { testBALSync(t, FullSync, sil.SIL71) }
func TestBALSynchronisationSnap(t *testing.T)       { testBALSync(t, SnapSync, sil.SIL71) }
func TestBALSynchronisationLegacyPeer(t *testing.T) { testBALSync(t, FullSync, sil.SIL69) }
...
func testBALSync(t *testing.T, mode SyncMode, protocol uint) {
	...
	success := make(chan struct{})
	tester := newTesterWithGenesis(t, mode, func() { close(success) }, false, gspec, beacon.New(silash.NewFaker()))
	defer tester.terminate()

	// Assemble the serving chain, executing the blocks to persist their access
	// lists.
	peerChain, err := core.NewBlockChain(rawdb.NewMemoryDatabase(), gspec, beacon.New(silash.NewFaker()), nil)
	if err != nil {
