...
type peerConnection struct {
	...
	version uint       // Sil protocol version number to switch strategies
...
type Peer interface {
	RequestHeadersByHash(common.Hash, int, int, bool, chan *sil.Response) (*sil.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *sil.Response) (*sil.Request, error)

	RequestBodies([]common.Hash, chan *sil.Response) (*sil.Request, error)
	RequestReceipts([]common.Hash, []uint64, []uint64, chan *sil.Response) (*sil.Request, error)
	RequestBALs([]common.Hash, chan *sil.Response) (*sil.Request, error)
}
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sil.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sil.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sil.ReceiptsMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBALRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sil.BlockAccessListsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sil.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sil.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sil.ReceiptsMsg, targetRTT)
...
func (p *peerConnection) BALCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sil.BlockAccessListsMsg, targetRTT)
	if cap > MaxBALFetch {
		cap = MaxBALFetch
	}
	return cap
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "sil")),
	}
}
