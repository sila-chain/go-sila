...
//
// Opposed to the original Sila block synchronization which is trustless (and
...
type skeleton struct {
	db     sildb.Database // Database backing the skeleton
...
func newSkeleton(db sildb.Database, peers *peerSet, drop peerDropFn, filler backfiller, chain chainReader) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		chain:      chain,
...
func (s *skeleton) saveSyncStatus(db sildb.KeyValueWriter) {
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, sil.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, sil.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*sil.BlockHeadersRequest)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, sil.BlockHeadersMsg, res.Time, len(headers))
