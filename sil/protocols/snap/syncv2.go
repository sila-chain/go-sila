...
// syncerV2 is an Sila account and storage trie syncer based on the snap
...
//
// The whole design is premised on SIP-6780 (self-destruct disabled): already
...
type syncerV2 struct {
	db     sildb.Database   // Database to store the trie nodes into (and dedup)
...
// newSyncerV2 creates a new snapshot syncer to download the Sila state over the
// snap protocol.
func newSyncerV2(db sildb.Database, scheme string) *syncerV2 {
	s := &syncerV2{
		db:     db,
		scheme: scheme,
...
func isPivotReorged(db sildb.Database, prev, curr *types.Header) bool {
...
func deleteRange(batch sildb.Batch, prefix []byte) {
...
// sync data!
func deleteKeyRange(batch sildb.Batch, start, limit []byte) {
...
func (s *syncerV2) resetTrienodes(batch sildb.Batch) {
...
func (s *syncerV2) processAccountResponse(res *accountResponseV2) {
	...
	for i, account := range res.accounts {
		...
		if account.Root != types.EmptyRootHash {
			...
			// change, guaranteed post SIP-6780.
	...
	// no longer possible in sila as self-destruction is disabled in SilaCancun
