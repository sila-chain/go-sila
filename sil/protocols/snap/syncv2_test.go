...
func verifyAdoptedSyncedState(scheme string, db sildb.KeyValueStore, root common.Hash, elems []*kv, t *testing.T) {
	t.Helper()
...
func decodeJournal(t *testing.T, db sildb.Database) *syncProgressV2 {
	t.Helper()
...
func assertAccountBalance(t *testing.T, db sildb.Database, hash common.Hash, want uint64) {
	t.Helper()
...
// returning.
func suspendChunkedContractSync(t *testing.T, db sildb.Database, scheme string, pivot *types.Header, accTrie *trie.Trie, accElems []*kv, stTrie *trie.Trie, stElems []*kv, contractHash common.Hash) {
	t.Helper()
...
// become empty post SIP-6780). The follow-up Sync must delete the downloaded
...
func testPivotMoveAbortsEmptiedStorage(t *testing.T, scheme string) {
	...
	// The A+1 BAL zeroes every slot: post SIP-6780 an emptied storage is
