...
type accountTask struct {
	...
	genBatch sildb.Batch // Batch used by the node generator
...
// syncer is an Sila account and storage trie syncer based on snapshots and
...
type syncer struct {
	db     sildb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        sildb.Batch        // Shared batch writer used for persisting raw states
...
func newSyncer(db sildb.KeyValueStore, scheme string) *syncer {
	return &syncer{
		db:     db,
		scheme: scheme,
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				...
				task.genBatch = sildb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = sildb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *syncer) commitHealer(force bool) {
	if !force && s.healer.scheduler.MemSize() < sildb.IdealBatchSize {
		return
	}
	batch := s.db.NewBatch()
...
func (s *syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > sildb.IdealBatchSize {
