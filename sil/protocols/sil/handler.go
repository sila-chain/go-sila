...
import (
	...
	"github.com/sila-chain/go-sila/p2p/enr"
...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `sil` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `sil`.
...
// NodeInfo represents a short summary of the `sil` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network uint64              `json:"network"` // Sila network ID (1=SilaMainnet, SilaHolesky=17000)
	Genesis common.Hash         `json:"genesis"` // SHA3 hash of the host's genesis block
	Config  *params.ChainConfig `json:"config"`  // Chain configuration for the fork rules
	Head    common.Hash         `json:"head"`    // Hex hash of the host's best owned block
}

// nodeInfo retrieves some `sil` protocol metadata about the running host node.
...
// Handle is invoked whenever an `sil` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `sil`", "err", err)
			return err
		}
	}
}
