...
func newTester(t *testing.T, confOverride func(*silconfig.Config)) *tester {
	// Create a temporary storage for the node keys and initialize it
	workspace := t.TempDir()

	// Create a networkless protocol stack and start an Sila service within
	...
	if confOverride != nil {
		confOverride(silConf)
	}
	silBackend, err := sil.New(stack, silConf)
	if err != nil {
		t.Fatalf("failed to register Sila protocol: %v", err)
	}
