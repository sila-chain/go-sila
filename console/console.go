...
var defaultAPIs = map[string]string{"sil": "1.0", "net": "1.0", "debug": "1.0"}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines sil.getBalance(sil.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		message += "at block: " + sil.blockNumber + " (" + new Date(1000 * sil.getBlock(sil.blockNumber).timestamp) + ")\n";
		try {
