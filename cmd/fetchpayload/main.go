...
// fetchpayload queries an Sila node over RPC, fetches a block and its
...
func main() {
	...
	// Parse block number (nil means "latest" in silclient).
	...
	// Connect to the node.
	client, err := silclient.DialContext(ctx, *rpcURL)
	if err != nil {
...
// Returns nil for "latest" (silclient convention for the head block).
