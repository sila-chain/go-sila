...
func TestHistoryImportAndExport(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Generate chain.
			db, blocks, _ := core.GenerateChainWithGenesis(genesis, silash.NewFaker(), int(count), func(i int, g *core.BlockGen) {
				if i == 0 {
					return
				}
			...
			chain, err := core.NewBlockChain(db, genesis, silash.NewFaker(), nil)
			if err != nil {
			...
			imported, err := core.NewBlockChain(db2, genesis, silash.NewFaker(), nil)
			if err != nil {
				t.Fatalf("unable to initialize chain: %v", err)
			}
