...
// exportBlock is the on-disk representation used by sila export and import.
//
...
// SIP-7928 data. Older export files, which end at Withdrawals, remain valid.
...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	go func() {
		...
		minFreeDiskSpace := 2 * silconfig.Defaults.TrieDirtyCache // Default 2 * 256Mb
...
func ImportPreimages(db sildb.Database, fn string) error {
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db sildb.Database, fn string) error {
...
func NewStateIterator(triedb *triedb.Database, db sildb.Database, root common.Hash) (*StateIterator, error) {
...
func ExportSnapshotPreimages(chaindb sildb.Database, stateIt *StateIterator, fn string, root common.Hash) error {
	log.Info("Exporting preimages", "file", fn)
...
type exportHeader struct {
	Magic    string // Always set to 'gsildbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "gsildbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
...
func ImportLDBData(db sildb.Database, f string, startIndex int64, interrupt chan struct{}) error {
