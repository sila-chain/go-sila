...
// NewSuite creates and returns a new sil-test suite that can
...
func (s *Suite) SilTests() []utesting.Test {
...
func (s *Suite) TestStatus(t *utesting.T) {
	t.Log(`This test is just a sanity check. It performs an sil protocol handshake.`)
...
func (s *Suite) TestGetBlockHeaders(t *utesting.T) {
	...
	req := &sil.GetBlockHeadersPacket{
		...
		GetBlockHeadersRequest: &sil.GetBlockHeadersRequest{
			Origin:  sil.HashOrNumber{Hash: s.chain.blocks[1].Hash()},
			Amount:  2,
			Skip:    1,
			Reverse: false,
		},
	...
	if err := conn.Write(silProto, sil.GetBlockHeadersMsg, req); err != nil {
		t.Fatalf("could not write to connection: %v", err)
	}
	headers := new(sil.BlockHeadersPacket)
	if err := conn.ReadMsg(silProto, sil.BlockHeadersMsg, &headers); err != nil {
...
to check if the node disconnects after receiving multiple invalid requests.`)
	...
	badReq := &sil.GetBlockHeadersPacket{
		GetBlockHeadersRequest: &sil.GetBlockHeadersRequest{
			Origin:  sil.HashOrNumber{Number: ^uint64(0)},
			Amount:  1,
			Skip:    0,
			Reverse: false,
		},
	...
	for i := 0; i < 10; i++ {
		...
		if err := conn.Write(silProto, sil.GetBlockHeadersMsg, badReq); err != nil {
			if err == errDisc {
...
concurrently, with different request IDs.`)
	...
	req1 := &sil.GetBlockHeadersPacket{
		RequestId: uint64(111),
		GetBlockHeadersRequest: &sil.GetBlockHeadersRequest{
			Origin: sil.HashOrNumber{
	...
	req2 := &sil.GetBlockHeadersPacket{
	...
	// Send both requests.
	if err := conn.Write(silProto, sil.GetBlockHeadersMsg, req1); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
	if err := conn.Write(silProto, sil.GetBlockHeadersMsg, req2); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}

	// Wait for responses.
	// Note they can arrive in either order.
	resp, err := collectHeaderResponses(conn, 2, func(msg *sil.BlockHeadersPacket) uint64 {
...
same request ID. The node should handle the request by responding to both requests.`)
	...
	reqID := uint64(1234)
	request1 := &sil.GetBlockHeadersPacket{
		RequestId: reqID,
		GetBlockHeadersRequest: &sil.GetBlockHeadersRequest{
			Origin: sil.HashOrNumber{
				Number: 1,
	...
	request2 := &sil.GetBlockHeadersPacket{
		RequestId: reqID,
		GetBlockHeadersRequest: &sil.GetBlockHeadersRequest{
			Origin: sil.HashOrNumber{
				Number: 33,
	...
	// Send the requests.
	if err = conn.Write(silProto, sil.GetBlockHeadersMsg, request1); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
	if err = conn.Write(silProto, sil.GetBlockHeadersMsg, request2); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
...
func (s *Suite) checkHeadersAgainstChain(req *sil.GetBlockHeadersPacket, resp *sil.BlockHeadersPacket) error {
...
func collectHeaderResponses(conn *Conn, n int, identity func(*sil.BlockHeadersPacket) uint64) (map[uint64]*sil.BlockHeadersPacket, error) {
	resp := make(map[uint64]*sil.BlockHeadersPacket, n)
	for range n {
		r := new(sil.BlockHeadersPacket)
		if err := conn.ReadMsg(silProto, sil.BlockHeadersMsg, r); err != nil {
...
and expects a response.`)
	...
	req := &sil.GetBlockHeadersPacket{
		GetBlockHeadersRequest: &sil.GetBlockHeadersRequest{
			Origin: sil.HashOrNumber{Number: 0},
			Amount: 2,
		},
	}
	// Read headers response.
...
func (s *Suite) TestGetBlockBodies(t *utesting.T) {
	...
	req := &sil.GetBlockBodiesPacket{
		RequestId: 55,
		GetBlockBodiesRequest: sil.GetBlockBodiesRequest{
			s.chain.blocks[54].Hash(),
			s.chain.blocks[75].Hash(),
		},
	}
	if err := conn.Write(silProto, sil.GetBlockBodiesMsg, req); err != nil {
		t.Fatalf("could not write to connection: %v", err)
	}
	// Wait for response.
	resp := new(sil.BlockBodiesPacket)
	if err := conn.ReadMsg(silProto, sil.BlockBodiesMsg, &resp); err != nil {
...
func (s *Suite) TestMaliciousHandshake(t *utesting.T) {
	...
	var (
		...
		version = sil.ProtocolVersions[0]
	)
	...
	for _, handshake := range handshakes {
		...
		if err := conn.Write(silProto, handshakeMsg, handshake); err != nil {
			t.Fatalf("could not write to connection: %v", err)
		}
...
func (s *Suite) TestBlockRangeUpdateInvalid(t *utesting.T) {
	...
	conn.Write(silProto, sil.BlockRangeUpdateMsg, &sil.BlockRangeUpdatePacket{
		EarliestBlock:   10,
		LatestBlock:     8,
...
on another peer connection using GetPooledTransactions.`)
	...
	req := &sil.GetPooledTransactionsPacket{
		RequestId:                    1234,
		GetPooledTransactionsRequest: hashes,
	}
	if err = conn.Write(silProto, sil.GetPooledTransactionsMsg, req); err != nil {
		t.Fatalf("could not write to conn: %v", err)
	}
	// Check that all received transactions match those that were sent to node.
	msg := new(sil.PooledTransactionsPacket)
	if err := conn.ReadMsg(silProto, sil.PooledTransactionsMsg, &msg); err != nil {
...
the transactions using a GetPooledTransactions request.`)
	...
	err = conn.Write(silProto, eth.NewPooledTransactionHashesMsg, ann)
	if err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
	...
	for {
		...
		switch msg := msg.(type) {
		case *sil.GetPooledTransactionsPacket:
...
func (s *Suite) TestBlobViolations(t *utesting.T) {
	...
	for _, test := range []struct {
		...
		// Invalid tx size.
		{
			...
			resp: sil.PooledTransactionsResponse(t1),
		},
		// Wrong tx type.
		{
			ann: eth.NewPooledTransactionHashesPacket71{
			...
			resp: sil.PooledTransactionsResponse(t2),
		},
	} {
		conn, err := s.dial()
		if err != nil {
		...
		if err := conn.Write(silProto, sil.NewPooledTransactionHashesMsg, test.ann); err != nil {
			t.Fatalf("sending announcement failed: %v", err)
		}
		req := new(sil.GetPooledTransactionsPacket)
		if err := conn.ReadMsg(silProto, sil.GetPooledTransactionsMsg, req); err != nil {
			t.Fatalf("reading pooled tx request failed: %v", err)
		}

		encTxs, _ := rlp.EncodeToRawList(test.resp)
		resp := sil.PooledTransactionsPacket{RequestId: req.RequestId, List: encTxs}
		if err := conn.Write(silProto, sil.PooledTransactionsMsg, resp); err != nil {
			t.Fatalf("writing pooled tx response failed: %v", err)
		}
		if code, _, err := conn.Read(); err != nil {
			t.Fatalf("expected disconnect on blob violation, got err: %v", err)
		} else if code != discMsg {
			if code == protoOffset(silProto)+sil.NewPooledTransactionHashesMsg {
...
// readUntil reads sil protocol messages until a message of the target type is
...
// readUntilDisconnect reads sil protocol messages until the peer disconnects.
...
func (s *Suite) testBadBlobTx(t *utesting.T, tx *types.Transaction, badTx *types.Transaction) {
	...
	badPeer := func() {
		...
		if err := conn.Write(silProto, sil.NewPooledTransactionHashesMsg, ann); err != nil {
			errc <- fmt.Errorf("sending announcement failed: %v", err)
			return
		}

		req, err := readUntil[sil.GetPooledTransactionsPacket](context.Background(), conn)
		if err != nil {
	...
	goodPeer := func() {
		...
		var req *sil.GetPooledTransactionsPacket
		ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
		defer cancel()

		req, err = readUntil[sil.GetPooledTransactionsPacket](ctx, conn)
		if err != nil {
