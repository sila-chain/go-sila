...
func (c *Conn) ReadEth() (any, error) {
	...
	for {
		...
		if getProto(code) != silProto {
			// Read until sil message.
			continue
		}
		code -= baseProtoLen
		...
		if err := rlp.DecodeBytes(data, msg); err != nil {
			return nil, fmt.Errorf("unable to decode sil msg: %v", err)
		}
		return msg, nil
	}
}
...
func (s *Suite) dialAndPeer(status *sil.StatusPacket) (*Conn, error) {
	c, err := s.dial()
	if err != nil {
		return nil, err
	}
...
func (c *Conn) peer(chain *Chain, status *sil.StatusPacket) error {
...
func (c *Conn) statusExchange(chain *Chain, status *sil.StatusPacket) error {
loop:
	for {
		code, data, err := c.Read()
		if err != nil {
		...
		switch code {
		case sil.StatusMsg + protoOffset(silProto):
			...
			for _, cap := range c.caps {
				if cap.Name == "sil" && cap.Version == uint(msg.ProtocolVersion) {
					break loop
				}
			}
	...
	if c.negotiatedProtoVersion == 0 {
		return errors.New("sil protocol version must be set in Conn")
	}
	if status == nil {
		// default status message
		status = &sil.StatusPacket{
	...
	if err := c.Write(silProto, sil.StatusMsg, status); err != nil {
		return fmt.Errorf("write to connection failed: %v", err)
	}
	return nil
}
