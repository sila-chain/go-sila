...
Use `devp2p enrdump <base64>` to verify and display an Sila Node Record.
...
### Sil Protocol Test Suite

The Sil Protocol test suite is a conformance test suite for the [sil protocol][sil].

To run the sil protocol test suite against your implementation, the node needs to be initialized
with our test chain. The chain files are located in `./cmd/devp2p/internal/siltest/testdata`.
...
The test suite can now be executed using the devp2p tool.
    ...
    devp2p rlpx sil-test \
        --chain internal/siltest/testdata   \
        --node enode://....                 \
