...
import (
	...
	"github.com/sila-chain/go-sila/accounts/scwallet"
	"github.com/sila-chain/go-sila/accounts/usbwallet"
	"github.com/sila-chain/go-sila/beacon/blsync"
...
var deprecatedConfigFields = map[string]bool{
	...
	"silconfig.Config.LightServ":               true,
	"silconfig.Config.LightIngress":            true,
	"silconfig.Config.LightEgress":             true,
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "sil")
	cfg.WSModules = append(cfg.WSModules, "sil")
...
func constructDevModeBanner(ctx *cli.Context, cfg gsilConfig) string {
  ...
  3. A random, pre-allocated developer account will be available and unlocked as
     sil.coinbase, which can be used for testing. The random dev account is temporary,
...
`
 ...
 Running in ephemeral mode.  The following account has been prefunded in the genesis:
       ...
       ------------------
       0x%x (10^49 SIL)
`, cfg.Sil.Miner.PendingFeeRecipient)
		if cfg.Sil.Miner.PendingFeeRecipient == utils.DeveloperAddr {
			devModeBanner += fmt.Sprintf(`
       Private Key
       ------------------
       0x%x
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if ctx.IsSet(utils.OverrideSilaOsaka.Name) {
	...
	filterSystem := utils.RegisterFilterAPI(stack, backend, &cfg.Sil)
	...
	utils.RegisterSyncOverrideService(stack, sil, syncConfig)

	if ctx.Bool(utils.DeveloperFlag.Name) {
		// Start dev mode.
		simBeacon, err := catalyst.NewSimulatedBeacon(ctx.Uint64(utils.DeveloperPeriodFlag.Name), cfg.Sil.Miner.PendingFeeRecipient, sil)
		if err != nil {
		...
		// Start blsync mode.
		srv := rpc.NewServer()
		srv.RegisterName("engine", catalyst.NewConsensusAPI(sil))
		...
		err := catalyst.Register(stack, sil)
		if err != nil {
