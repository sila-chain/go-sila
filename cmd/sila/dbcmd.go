...
of sila that use the pebble v1 library.`,
	}
)
...
func removeDB(ctx *cli.Context) error {
	...
	var (
		...
		ancientDir = config.Sil.DatabaseFreezer
	)
	switch {
	case ancientDir == "":
...
func showDBStats(db sildb.KeyValueStater) {
	stats, err := db.Stat()
	if err != nil {
...
type snapshotIterator struct {
	init    bool
	account sildb.Iterator
	storage sildb.Iterator
}
...
var chainExporters = map[string]func(db sildb.Database) utils.ChainDataIterator{
	"preimage": func(db sildb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db sildb.Database) utils.ChainDataIterator {
		account := db.NewIterator(rawdb.SnapshotAccountPrefix, nil)
		storage := db.NewIterator(rawdb.SnapshotStoragePrefix, nil)
		return &snapshotIterator{account: account, storage: storage}
	},
	"code": func(db sildb.Database) utils.ChainDataIterator {
