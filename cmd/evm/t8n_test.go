...
func TestT8n(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		},
		{
			base: "./testdata/1",
			input: t8nInput{
				"alloc.json", "txs.json", "env.json", "SilaByzantium", "",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
		{ // blockhash test
			base: "./testdata/3",
			input: t8nInput{
				"alloc.json", "txs.json", "env.json", "SilaBerlin", "",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
		...
		{ // Uncle test
			base: "./testdata/5",
			input: t8nInput{
				"alloc.json", "txs.json", "env.json", "SilaByzantium", "0x80",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
		...
		{ // Already signed transactions
			base: "./testdata/13",
			input: t8nInput{
				"alloc.json", "signed_txs.rlp", "env.json", "SilaLondon", "",
			},
		...
		{ // Difficulty calculation - with uncles
			base: "./testdata/14",
			input: t8nInput{
				"alloc.json", "txs.json", "env.uncles.json", "SilaLondon", "",
			},
			output: t8nOutput{result: true},
			expOut: "exp2.json",
		},
		{ // Difficulty calculation - with ommers + SilaBerlin
			base: "./testdata/14",
			input: t8nInput{
				"alloc.json", "txs.json", "env.uncles.json", "SilaBerlin", "",
			},
			output: t8nOutput{result: true},
		...
		{ // Test withdrawals transition
			base: "./testdata/26",
			input: t8nInput{
				"alloc.json", "txs.json", "env.json", "SilaShanghai", "",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
		{ // SilaCancun tests
			base: "./testdata/28",
			input: t8nInput{
				"alloc.json", "txs.rlp", "env.json", "SilaCancun", "",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
		...
		{ // SilaPrague test, SIP-7702 transaction
			base: "./testdata/33",
			input: t8nInput{
				"alloc.json", "txs.json", "env.json", "SilaPrague", "",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
		{ // SilaOsaka test, SIP-7918 blob gas with parent base fee
			base: "./testdata/34",
			input: t8nInput{
				"alloc.json", "txs.json", "env.json", "SilaOsaka", "",
			},
			output: t8nOutput{alloc: true, result: true},
			expOut: "exp.json",
		},
	...
	for i, tc := range []struct {
		...
		{ // SilaLondon txs on sila_homestead
			...
			input: t9nInput{
				...
				stFork: "SilaHomestead",
			},
			expOut: "exp.json",
		},
		{ // SilaLondon txs on SilaLondon
			base: "./testdata/15",
			input: t9nInput{
				inTxs:  "signed_txs.rlp",
				stFork: "SilaLondon",
...
func (args *b11rInput) get(base string) []string {
	...
	if args.silash {
		out = append(out, "--seal.silash")
	}
	if opt := args.silashMode; opt != "" {
		out = append(out, "--seal.silash.mode")
		out = append(out, fmt.Sprintf("%v/%v", base, opt))
	}
	if opt := args.silashDir; opt != "" {
		out = append(out, "--seal.silash.dir")
		out = append(out, fmt.Sprintf("%v/%v", base, opt))
	}
	...
	for i, tc := range []struct {
	...
	}{
		{
			...
			input: []string{"t8n",
				...
				"--input.env=./testdata/31/env.json", "--state.fork=SilaCancun",
				"--trace",
			},
