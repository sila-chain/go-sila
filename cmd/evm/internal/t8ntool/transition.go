...
import (
	...
	"github.com/sila-chain/go-sila/core/overlay"
...
func Transition(ctx *cli.Context) error {
	...
	if err := applySilaLondonChecks(&prestate.Env, chainConfig); err != nil {
		return err
	}
	if err := applySilaShanghaiChecks(&prestate.Env, chainConfig); err != nil {
		return err
	}
	if err := applyMergeChecks(&prestate.Env, chainConfig); err != nil {
		return err
	}
	if err := applySilaCancunChecks(&prestate.Env, chainConfig); err != nil {
		return err
	}
...
func applySilaLondonChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	if !chainConfig.IsSilaLondon(big.NewInt(int64(env.Number))) {
		return nil
	}
...
func applySilaShanghaiChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	if !chainConfig.IsSilaShanghai(big.NewInt(int64(env.Number)), env.Timestamp) {
		return nil
	}
	if env.Withdrawals == nil {
		return NewError(ErrorConfig, errors.New("SilaShanghai config but missing 'withdrawals' in env section"))
	}
	return nil
}
...
func applySilaCancunChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	if !chainConfig.IsSilaCancun(big.NewInt(int64(env.Number)), env.Timestamp) {
