...
Golang execution layer implementation of the Sila protocol.
...
The go-sila project comes with several wrappers/executables found in the `cmd`
directory.
...
By far the most common scenario is people wanting to simply interact with the Sila
...
accounts, `sila` will by default correctly separate the two networks and will not make any
accounts available between them.*

### Configuration

As an alternative to passing the numerous flags to the `sila` binary, you can also pass a
...
One of the quickest ways to get Sila up and running on your machine is by using
Docker:
...
This will start `sila` in snap-sync mode with a DB memory allowance of 1GB, as the
above command does.  It will also create a persistent volume in your home directory for
saving your blockchain as well as map the default ports. There is also an `alpine` tag
available for a slim version of the image.

Do not forget `--http.addr 0.0.0.0`, if you want to access RPC from other containers
and/or hosts. By default, `sila` binds to the local interface and RPC endpoints are not
...
The IPC interface is enabled by default and exposes all the APIs supported by `sila`,
...
HTTP based JSON-RPC API options:
  ...
  * `--http.api` API's offered over the HTTP-RPC interface (default: `sil,net,web3`)
  * `--http.corsdomain` Comma separated list of domains from which to accept cross-origin requests (browser enforced)
  * `--ws` Enable the WS-RPC server
  * `--ws.addr` WS-RPC server listening interface (default: `localhost`)
  * `--ws.port` WS-RPC server listening port (default: `8546`)
  * `--ws.api` API's offered over the WS-RPC interface (default: `sil,net,web3`)
  * `--ws.origins` Origins from which to accept WebSocket requests
  * `--ipcdisable` Disable the IPC-RPC server
  * `--ipcpath` Filename for IPC socket/pipe within the datadir (explicit paths escape it)

You'll need to use your own programming environments' capabilities (libraries, tools, etc) to
connect via HTTP, WS or IPC to a `sila` node configured with the above flags and you'll
need to speak [JSON-RPC](https://www.jsonrpc.org/specification) on all transports. You
can reuse the same connection for multiple requests!

**Note: Please understand the security implications of opening up an HTTP/WS based
transport before doing so! Hackers on the internet are actively trying to subvert
Sila nodes with exposed APIs! Further, all browser tabs can access locally
...
Unfortunately since [the Merge](https://sila.org/en/roadmap/merge/) it is no longer possible
to easily set up a network of sila nodes without also setting up a corresponding beacon chain.
...
If you'd like to contribute to go-sila, please fork, fix, commit and send a pull request
...
The go-sila library (i.e. all code outside of the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html),
also included in our repository in the `COPYING.LESSER` file.

The go-sila binaries (i.e. all code inside of the `cmd` directory) are licensed under the
