...
func (ec *Client) CreateAccessList(ctx context.Context, msg sila.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "sil_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "sil_getProof", account, keys, toBlockNumArg(blockNumber))
	if err != nil {
		return nil, err
	}
...
func (ec *Client) CallContract(ctx context.Context, msg sila.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	...
	err := ec.c.CallContext(
		ctx, &hex, "sil_call", toCallArg(msg),
...
// GCStats retrieves the current garbage collection stats from a sila node.
...
// MemStats retrieves the current memory stats from a sila node.
...
// GetNodeInfo retrieves the node info of a sila node.
...
func (ec *Client) SubscribeFullPendingTransactions(ctx context.Context, ch chan<- *types.Transaction) (*rpc.ClientSubscription, error) {
	return ec.c.SilSubscribe(ctx, ch, "newPendingTransactions", true)
}
...
// OverrideAccount is an alias for sila.OverrideAccount.
type OverrideAccount = sila.OverrideAccount

// BlockOverrides is an alias for sila.BlockOverrides.
