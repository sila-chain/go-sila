...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block, []common.Hash) {
	...
	n, err := node.New(&node.Config{
		HTTPModules: []string{"debug", "sil", "admin"},
	})
	if err != nil {
		t.Fatalf("can't create new node: %v", err)
	}
	// Create Sila Service
	config := &silconfig.Config{Genesis: genesis, RPCGasCap: 1000000}
	ethservice, err := sil.New(n, config)
	if err != nil {
...
func generateTestChain() (*core.Genesis, []*types.Block, []common.Hash) {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, silash.NewFaker(), 1, generate)
...
func testGetProof(t *testing.T, client *rpc.Client, addr common.Address) {
	ec := New(client)
	ethcl := silclient.NewClient(client)
