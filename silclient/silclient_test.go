...
var (
	_ = sila.ChainReader(&silclient.Client{})
	_ = sila.TransactionReader(&silclient.Client{})
	_ = sila.ChainStateReader(&silclient.Client{})
	_ = sila.ChainSyncReader(&silclient.Client{})
	_ = sila.ContractCaller(&silclient.Client{})
	_ = sila.GasEstimator(&silclient.Client{})
	_ = sila.GasPricer(&silclient.Client{})
	_ = sila.LogFilterer(&silclient.Client{})
	_ = sila.PendingStateReader(&silclient.Client{})
	// _ = sila.PendingStateEventer(&silclient.Client{})
	_ = sila.PendingContractCaller(&silclient.Client{})
)
...
func newTestBackend(config *node.Config) (*node.Node, []*types.Block, error) {
	...
	// Create Sila Service
	ecfg := &silconfig.Config{Genesis: genesis, RPCGasCap: 1000000}
	ethservice, err := sil.New(n, ecfg)
	if err != nil {
...
func generateTestChain() []*types.Block {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, beacon.New(silash.NewFaker()), 2, generate)
...
func TestBlockReceiptsPreservesCanonicalFlag(t *testing.T) {
	...
	if err := srv.RegisterName("sil", service); err != nil {
...
func sendTransaction(ec *silclient.Client) error {
...
func ExampleRevertErrorData() {
	// First create an silclient.Client instance.
	ctx := context.Background()
	ec, _ := silclient.DialContext(ctx, exampleNode.HTTPEndpoint())
	...
	revertData, ok := silclient.RevertErrorData(err)
	if !ok {
...
func TestSimulateV1(t *testing.T) {
	...
	client := silclient.NewClient(backend.Attach())
	defer client.Close()
	...
	// Simple test: transfer SIL from one account to another
	from := testAddr
	...
	opts := silclient.SimulateOptions{
		BlockStateCalls: []silclient.SimulateBlock{
			{
				Calls: []sila.CallMsg{
					{
						From:      from,
...
func TestSimulateV1WithStateOverrides(t *testing.T) {
	...
	from := testAddr
	to := common.HexToAddress("0x0000000000000000000000000000000000000001")
	value := big.NewInt(1000000000000000000) // 1 SIL
	gas := uint64(100000)
	maxFeePerGas := new(big.Int).Mul(header.BaseFee, big.NewInt(2))
