...
func newCanonicalStore[T any](db sildb.Iteratee, keyPrefix []byte) (*canonicalStore[T], error) {
	cs := &canonicalStore[T]{
		keyPrefix: keyPrefix,
...
func (cs *canonicalStore[T]) add(backend sildb.KeyValueWriter, period uint64, value T) error {
...
func (cs *canonicalStore[T]) deleteFrom(db sildb.KeyValueWriter, fromPeriod uint64) (deleted periodRange) {
...
func (cs *canonicalStore[T]) get(backend sildb.KeyValueReader, period uint64) (T, bool) {
	var null, value T
