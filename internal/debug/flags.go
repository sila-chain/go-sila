...
var (
	...
	logVmoduleFlag = &cli.StringFlag{
		Name:     "log.vmodule",
		Usage:    "Per-module verbosity: comma-separated list of <pattern>=<level> (e.g. sil/*=5,p2p=4)",
		Value:    "",
		Category: flags.LoggingCategory,
	}
...
func Setup(ctx *cli.Context) error {
	...
	if rotation {
		...
		// so typically /tmp/gsil-lumberjack.log on linux
		if len(logFile) > 0 {
			context = append(context, "location", logFile)
		} else {
			context = append(context, "location", filepath.Join(os.TempDir(), "gsil-lumberjack.log"))
		}
