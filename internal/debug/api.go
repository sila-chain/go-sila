...
func (*HandlerT) Stacks(filter *string) string {
	...
	if filter != nil && len(*filter) > 0 {
		...
		// interpreter:
		//
		// E.g. (sil || snap) && !p2p -> (sil in Value || snap in Value) && p2p not in Value
...
//
//   - Setting the limit too low will cause Sila to become unresponsive.
//
//   - Sila also allocates memory off-heap, particularly for fastCache and Pebble,
