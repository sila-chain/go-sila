...
func (args *TransactionArgs) setFeeDefaults(ctx context.Context, b Backend, head *types.Header) error {
	...
	if b.ChainConfig().IsSilaCancun(head.Number, head.Time) {
		args.setSilaCancunFeeDefaults(b.ChainConfig(), head)
	}
	...
	isSilaLondon := b.ChainConfig().IsSilaLondon(head.Number)
	if args.GasPrice != nil && !sip1559ParamsSet {
	...
	// Now attempt to fill in default value depending on whether SilaLondon is active or not.
	if isSilaLondon {
		// SilaLondon is active, set maxPriorityFeePerGas and maxFeePerGas.
		if err := args.setSilaLondonFeeDefaults(ctx, head, b); err != nil {
			return err
		}
	} else {
		if args.MaxFeePerGas != nil || args.MaxPriorityFeePerGas != nil {
			return errors.New("maxFeePerGas and maxPriorityFeePerGas are not valid before SilaLondon is active")
		}
		// SilaLondon not active, set gas price.
...
// setSilaCancunFeeDefaults fills in reasonable default fee values for unspecified fields.
func (args *TransactionArgs) setSilaCancunFeeDefaults(config *params.ChainConfig, head *types.Header) {
...
// setSilaLondonFeeDefaults fills in reasonable default fee values for unspecified fields.
func (args *TransactionArgs) setSilaLondonFeeDefaults(ctx context.Context, head *types.Header, b Backend) error {
...
// for the purpose of sil_call class of RPC methods.
