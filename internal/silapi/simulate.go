...
// simOpts are the inputs to sil_simulateV1.
type simOpts struct {
...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, map[common.Hash]common.Address, error) {
	...
	if sim.chainConfig.IsSilaCancun(header.Number, header.Time) {
		var excess uint64
		if sim.chainConfig.IsSilaCancun(parent.Number, parent.Time) {
	...
	// Withdrawals are a post-SilaShanghai field. Attaching a non-nil withdrawals
	// slice would cause types.NewBlock to populate WithdrawalsHash on the
	// header and emit withdrawals fields for pre-SilaShanghai blocks.
...
func (sim *simulator) makeHeaders(blocks []simBlock) ([]*types.Header, error) {
	...
	for bi, block := range blocks {
		...
		if sim.chainConfig.IsSilaShanghai(number, timestamp) {
			withdrawalsHash = &types.EmptyWithdrawalsHash
		}

		var parentBeaconRoot *common.Hash
		if sim.chainConfig.IsSilaCancun(number, timestamp) {
