...
func (api *SilaAPI) BaseFee(ctx context.Context) *hexutil.Big {
...
// BlockChainAPI provides an API to access Sila blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Sila blockchain API.
...
// proofList implements sildb.KeyValueWriter and collects the proofs as
...
// Config implements the SIP-7910 sil_config method.
func (api *BlockChainAPI) Config(ctx context.Context) (*configResponse, error) {
	...
	assemble := func(c *params.ChainConfig, ts *uint64) *config {
		...
		var (
			rules       = c.Rules(c.SilaLondonBlock, true, t)
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if !b.UnprotectedAllowed() && !tx.Protected() {
		// Ensure only sip155 signed transactions are submitted if SIP155Required is set.
...
func (api *TransactionAPI) currentBlobSidecarVersion() byte {
	...
	if api.b.ChainConfig().IsSilaOsaka(h.Number, h.Time) {
...
func (api *TransactionAPI) SendRawTransaction(ctx context.Context, input hexutil.Bytes) (common.Hash, error) {
	...
	// TODO: remove in go-sila v1.17.x
...
//
// https://sila.org/en/developers/docs/apis/json-rpc/#sil_sign
...
// DebugAPI is the collection of Sila APIs exposed over the debugging
// namespace.
...
// Version returns the current sila protocol version.
