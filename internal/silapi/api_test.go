...
func TestTransactionBlobTx(t *testing.T) {
	...
	config.SilaShanghaiTime = new(uint64)
	config.SilaCancunTime = new(uint64)
...
func (b testBackend) SyncProgress(ctx context.Context) sila.SyncProgress {
...
func (b testBackend) ChainDb() sildb.Database                  { return b.db }
...
func TestEstimateGas(t *testing.T) {
	...
	api := NewBlockChainAPI(newTestBackend(t, genBlocks, genesis, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {
...
func TestSimulateV1(t *testing.T) {
	...
	api := NewBlockChainAPI(newTestBackend(t, genBlocks, genesis, silash.NewFaker(), func(i int, b *core.BlockGen) {
...
func TestSimulateV1ChainLinkage(t *testing.T) {
	...
	backend := newTestBackend(t, 1, gspec, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {
...
func TestSimulateV1TxSender(t *testing.T) {
	...
	backend := newTestBackend(t, 0, gspec, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {})
	stateDB, baseHeader, err := backend.StateAndHeaderByNumberOrHash(ctx, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber))
	if err != nil {
		t.Fatalf("failed to get state and header: %v", err)
	}
...
// post-SilaShanghai. Pre-SilaShanghai blocks must omit both fields, otherwise the
// header hash and size would not match a valid pre-SilaShanghai block.
...
func TestSignTransaction(t *testing.T) {
	...
	b := newTestBackend(t, 1, genesis, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS()
	})
...
func TestRPCMarshalBlock(t *testing.T) {
	...
	for i, tc := range testSuite {
		resp := RPCMarshalBlock(block, tc.inclTx, tc.fullTx, params.SilaMainnetChainConfig)
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	backend := newTestBackend(t, genBlocks, genesis, silash.NewFaker(), func(i int, b *core.BlockGen) {
		// Transfer from account[0] to account[1]
		//    value: 1000 wei
		//    fee:   0 wei
...
func setupReceiptBackend(t *testing.T, genBlocks int) (*testBackend, []common.Hash) {
	...
	backend := newTestBackend(t, genBlocks, genesis, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {
		var (
		...
		default:
			// transfer 1000wei
			tx, err = types.SignTx(types.NewTx(&types.LegacyTx{Nonce: uint64(i), To: &acc2Addr, Value: big.NewInt(1000), Gas: params.TxGas, GasPrice: b.BaseFee(), Data: nil}), types.SilaHomesteadSigner{}, acc1Key)
		}
		if err != nil {
...
func TestRPCGetTransactionReceipt(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "sil_getTransactionReceipt", tt.file)
	}
}
...
func TestRPCGetBlockReceipts(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "sil_getBlockReceipts", tt.file)
	}
}
...
func TestCreateAccessListWithStateOverrides(t *testing.T) {
	...
	backend := newTestBackend(t, 1, genesis, silash.NewFaker(), nil)
...
func TestEstimateGasWithMovePrecompile(t *testing.T) {
	...
	backend := newTestBackend(t, 1, genesis, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS()
	})
	api := NewBlockChainAPI(backend)
...
func TestEIP7910Config(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "sil_config", tt.file)
	}
}
...
// makeSelfSignedRaw is a convenience for a 0-SIL self-transfer.
...
func TestSendRawTransactionSync_Success(t *testing.T) {
	...
	b := newTestBackend(t, 0, genesis, silash.NewFaker(), nil)
...
func TestGetStorageValues(t *testing.T) {
	...
	api := NewBlockChainAPI(newTestBackend(t, 1, genesis, beacon.New(silash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS()
	}))
	latest := rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber)
...
func TestStateMethodsDefaultToLatest(t *testing.T) {
	...
	srv := rpc.NewServer()
	if err := srv.RegisterName("sil", NewBlockChainAPI(backend)); err != nil {
		t.Fatal(err)
	}
	if err := srv.RegisterName("sil", NewTransactionAPI(backend, new(AddrLocker))); err != nil {
		t.Fatal(err)
	}
