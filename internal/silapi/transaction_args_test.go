...
func TestSetFeeDefaults(t *testing.T) {
	...
	tests := []test{
		...
		{
			"legacy tx pre-SilaLondon with zero price",
			"legacy",
		...
		{
			"access list tx post-SilaLondon, explicit gas price",
			"legacy",
			&TransactionArgs{AccessList: al, GasPrice: fortytwo},
			&TransactionArgs{AccessList: al, GasPrice: fortytwo},
			nil,
		...
		{
			"dynamic fee tx pre-SilaLondon, maxFee set",
			"legacy",
			&TransactionArgs{MaxFeePerGas: maxFee},
			nil,
			errors.New("maxFeePerGas and maxPriorityFeePerGas are not valid before SilaLondon is active"),
		},
		{
			"dynamic fee tx pre-SilaLondon, priorityFee set",
			"legacy",
...
func (b *backendMock) SyncProgress(ctx context.Context) sila.SyncProgress {
...
func (b *backendMock) ChainDb() sildb.Database           { return nil }
