...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.SilSubscribe(ctx, resultChan, "foo")
	if err != nil {
	...
	// This is important for reproducing https://github.com/sila-chain/go-sila/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the sil_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","method":"sil_subscription","params":{"subscription":"0x00","result":1}}`
	)
